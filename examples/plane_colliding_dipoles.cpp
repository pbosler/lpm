...
int main(int argc, char* argv[]) {
  ...
  {  // Kokkos scope
    ...
    PolyMeshParameters<seed_type> mesh_params(
        ...
        input.get_option("mesh_radius").get_real(), amr_buffer, amr_limit);
    Real max_circ_tol = input.get_option("max_circulation_tol").get_real();
    Real flow_map_var_tol =
        input.get_option("flow_map_variation_tol").get_real();
    Real zeta_var_tol = input.get_option("vorticity_variation_tol").get_real();
    const bool amr =
        (mesh_params.amr_limit > 0 and
    ...
    const Real epsilon =
        input.get_option("kernel_smoothing_parameter").get_real();
    auto plane = std::make_unique<Incompressible2D<seed_type>>(
        mesh_params, coriolis, epsilon);
    ...
    if (amr) {
      ...
      ScalarIntegralFlag max_circulation_flag(
          refiner.flags, plane->rel_vort_active.view, plane->mesh.faces.area,
          plane->mesh.faces.mask, plane->mesh.n_faces_host(), max_circ_tol);
      FlowMapVariationFlag<seed_type> flow_map_variation_flag(
          refiner.flags, plane->mesh, flow_map_var_tol);
      ScalarVariationFlag zeta_var_flag(
          refiner.flags, plane->rel_vort_active.view,
          plane->rel_vort_passive.view, plane->mesh.faces.verts,
          plane->mesh.faces.mask, plane->mesh.n_faces_host(), zeta_var_tol);
      ...
      logger.info(
          "amr is enabled with limit {}, max_circ_tol = {}, flow_map_var_tol = "
          "{}, zeta_var_tol = {}",
          mesh_params.amr_limit, max_circ_tol, flow_map_var_tol, zeta_var_tol);
      ...
      for (int i = 0; i < amr_limit; ++i) {
        ...
        logger.info(
            "amr iteration {}: initial circulation refinement count = {}", i,
            refiner.count[0]);
        logger.info(
            "amr iteration {}: vorticity variation refinement count = {}", i,
            refiner.count[1]);
    ...
    const auto vel_range =
        plane->velocity_active.range(plane->mesh.n_faces_host());
    const Real cr = vel_range.second * dt / plane->mesh.appx_min_mesh_size();
    logger.info(plane->info_string());
    logger.info(
        "velocity magnitude (min, max) = ({}, {}); approximate Courant number "
        "= {}",
    ...
    for (int t_idx = 0; t_idx < nsteps; ++t_idx) {
      ...
      if (do_remesh) {
        ...
        if (interval_trigger) {
          ...
          logger.info("remesh {} triggered by ftle_tol {}", remesh_counter,
                      ftle_tol);
        }

        auto new_plane = std::make_unique<Incompressible2D<seed_type>>(
            mesh_params, coriolis, epsilon);
        ...
        if (amr) {
          ...
          ScalarIntegralFlag max_circulation_flag(
              refiner.flags, new_plane->rel_vort_active.view,
              new_plane->mesh.faces.area, new_plane->mesh.faces.mask,
              new_plane->mesh.n_faces_host(), max_circ_tol);

          FlowMapVariationFlag<seed_type> flow_map_variation_flag(
              refiner.flags, new_plane->mesh, flow_map_var_tol);

          ScalarVariationFlag zeta_var_flag(
              refiner.flags, new_plane->rel_vort_active.view,
              new_plane->rel_vort_passive.view, new_plane->mesh.faces.verts,
              new_plane->mesh.faces.mask, new_plane->mesh.n_faces_host(),
              zeta_var_tol);

          if (remesh_strategy == "direct") {
            remesh.adaptive_direct_remesh(refiner, max_circulation_flag,
                                          zeta_var_flag,
                                          flow_map_variation_flag);
          } else {
            remesh.adaptive_indirect_remesh(vorticity, coriolis, refiner,
                                            zeta_var_flag, max_circulation_flag,
                                            flow_map_variation_flag);
          }
        } else {
          if (remesh_strategy == "direct") {
        ...
        plane->update_device();
        auto new_solver = std::make_unique<Incompressible2DRK2<seed_type>>(
            dt, *plane, solver->t_idx);
      ...
      Kokkos::parallel_for(
          ...
          ComputeFTLE<seed_type>(
              plane->ftle.view, plane->mesh.vertices.phys_crds.view,
              plane->ref_crds_passive.view, plane->mesh.faces.phys_crds.view,
              plane->ref_crds_active.view, plane->mesh.faces.verts,
              plane->mesh.faces.mask, plane->t - tref));
      max_ftle = get_max_ftle(plane->ftle.view, plane->mesh.faces.mask,
                              plane->mesh.n_faces_host());
...
void input_init(user::Input& input) {
  ...
  user::Option vorticity_variation_option(
      "vorticity_variation_tol", "-z", "--zeta-variation",
      "amr max relative vorticity variation tolerance",
      std::numeric_limits<Real>::max());
  input.add_option(vorticity_variation_option);

  user::Option flow_map_variation_option(
      "flow_map_variation_tol", "-fv", "--flow-map-variation",
      "amr max flow map variation tolerance", std::numeric_limits<Real>::max());
  input.add_option(flow_map_variation_option);

  user::Option output_file_directory_option(
      "output_file_directory", "-odir", "--output-dir", "output file directory",
      std::string("."));
