...
template <typename SeedType>
void tc2_exact_sol(StaggeredSWE<SeedType, ZeroFunctor>& swe,
                   const CoriolisSphere& coriolis);
...
int main(int argc, char* argv[]) {
  ...
  {  // Kokkos scope
    user::Input input("sphere_tc2");
    {
      ...
      user::Option gmls_poynomial_order_option("gmls_polynomial_order", "-g",
                                               "--gmls-order",
                                               "gmls polynomial order", 4);
      input.add_option(gmls_poynomial_order_option);

      user::Option verbose_output_option("verbose", "-v", "--verbose",
                                         "verbose output to logger", false);
    ...
    PolyMeshParameters<seed_type> mesh_params(
        input.get_option("tree_depth").get_int(), sphere_radius, amr_limit);
    coriolis_type coriolis(omega);
    topography_type topo;
    auto sphere = std::make_unique<StaggeredSWE<seed_type, topography_type>>(
        mesh_params, coriolis, topo);
    ...
    divergence_type div;
    gmls::Params gmls_sfc_lap_params(
        input.get_option("gmls_polynomial_order").get_int());
    sphere->init_fields(sfc, vorticity, div, gmls_sfc_lap_params);

    tc2_setup_tracers(*sphere);
    tc2_exact_sol(*sphere, coriolis);

    logger.debug("found {} nans in velocity field",
                 sphere->velocity.nan_count(sphere->mesh.n_vertices_host()));
    logger.debug("found {} nans in double_dot field",
                 sphere->double_dot.nan_count(sphere->mesh.n_vertices_host()));
    logger.debug("found {} nans in double_dot_avg field",
                 sphere->double_dot_avg.nan_count(sphere->mesh.n_faces_host()));
    ...
    */
    auto solver =
        std::make_unique<solver_type>(dt, *sphere, gmls_sfc_lap_params);
...
void tc2_exact_sol(StaggeredSWE<SeedType, ZeroFunctor>& swe,
  ...
  Kokkos::parallel_for(
      swe.mesh.n_faces_host(), KOKKOS_LAMBDA(const Index i) {
