...
int main(int argc, char* argv[]) {
  ...
  ko::initialize(argc, argv);
  {
    ...
    */
    ...
    const auto src_data =
        reader.create_scalar_field(input.field_name, input.source_timestep);
    ...
    */
    ...
    auto mesh_params             = PolyMeshParameters<seed_type>(
        input.init_depth, sphere_radius, input.amr_refinement_buffer);

    auto sphere = std::make_unique<PolyMesh2d<seed_type>>(mesh_params);
    sphere->update_device();
    ScalarField<VertexField> gmls_verts(input.field_name,
    ...
    */
        ...
        {Compadre::ScalarPointEvaluation});
    {
      ...
      gmls::Neighborhoods neighbors(src_coords.get_host_crd_view(),
                                    gather.h_phys_crds, gmls_params);
      auto scalar_gmls = gmls::sphere_scalar_gmls(
          src_coords.get_host_crd_view(), gather.phys_crds, neighbors,
      ...
      gather.scalar_fields.at(input.field_name) =
          gmls_eval
              .applyAlphasToDataAllComponentsAllTargetSites<Real*, DevMemory>(
                  src_data.view, Compadre::ScalarPointEvaluation,
      ...
      */
      ...
      Kokkos::parallel_reduce(
          ...
          KOKKOS_LAMBDA(const Index i,
                        typename Kokkos::MinMax<Real>::value_type& mm) {
      ...
      const Real mass_tol = input.scalar_integral_tol * minmax.max_val *
                            square(sphere->appx_mesh_size());
      const Real var_tol =
          input.scalar_var_tol * (minmax.max_val - minmax.min_val);
      ...
      for (int i = 0; i < input.amr_refinement_limit; ++i) {
        ...
        if (input.scalar_max_tol > 0) {
          Kokkos::parallel_for(
              range, ScalarMaxFlag(flags, gmls_faces.view, sphere->faces.mask,
                                   sphere->n_faces_host(), max_tol));
        }
        if (input.scalar_integral_tol > 0) {
          Kokkos::parallel_for(
              range, ScalarIntegralFlag(flags, gmls_faces.view,
                                        sphere->faces.area, sphere->faces.mask,
                                        sphere->n_faces_host(), mass_tol));
        }
        if (input.scalar_var_tol > 0) {
          Kokkos::parallel_for(
              range,
              ScalarVariationFlag(flags, gmls_faces.view, gmls_verts.view,
                                  sphere->faces.verts, sphere->faces.mask,
                                  sphere->n_faces_host(), var_tol));
        }
...
Input::Input(int argc, char* argv[]) {
  ...
  bool amr_valid =
      ((amr_refinement_buffer == 0 and amr_refinement_limit == 0) or
       (amr_refinement_buffer > 0 and amr_refinement_limit > 0));
  LPM_REQUIRE_MSG(amr_valid,
                  "valid input for amr requires both refinement buffer and "
                  "refinement limit to be zero (no amr) or to be positive");
  ofroot +=
      (std::is_same<CubedSphereSeed, seed_type>::value ? "cubed_sphere"
                                                       : "icos_tri_sphere") +
      std::to_string(init_depth) +
      (amr_refinement_buffer > 0
           ? "_amr" + std::to_string(amr_refinement_buffer)
           : "_unif") +
...
std::string Input::usage() const {
  ...
  ss << "Sphere Mesh from data: This program initializes a uniform spherical "
        "mesh \n"
  ...
  ss << "\t   "
     << "-d [nonnegative integer] ; defines the initial depth of the uniform "
        "mesh's face quadtree.\n";
  ss << "\t   " << "-f --field-name ; name of the variable to interpolate\n";
  ss << "\t   " << "-g --gmls-order ; order of the GMLS basis polynomials\n";
  ss << "\t   "
     << "-amr [nonnegative integer]; sets both amr buffer and amr limit\n";
  ss << "\t   "
     << "--amr-limit [nonnegative integer]; sets the maximum number of times a "
        "panel may be refined relative to its immediate neighbors; "
        "--amr-buffer must be positive whenever this option is positive";
  ss << "\t   "
     << "--amr-buffer [nonnegative integer]; sets maximum memory limit for the "
        "particle/panel mesh to be a uniform mesh of depth init-depth + "
        "buffer.  --amr-limit must be positive whenever --amr-buffer is "
        "positive\n";
  ss << "\t   "
     << "--max-tol sets maximum value threshold for scalar data amr\n";
  ss << "\t   "
     << "--integral-tol sets maximum integral threshold for scalar data amr\n";
  ss << "\t   "
     << "--var-tol sets maximum variation threshold for scalar data amr\n";
...
std::string Input::info_string() const {
  ...
  if (amr_refinement_buffer > 0) {
    ss << "\tAMR:\n";
    ss << "\t\t(buffer, limit) = (" << amr_refinement_buffer << ", "
       << amr_refinement_limit << ")\n";
