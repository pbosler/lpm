...
KOKKOS_INLINE_FUNCTION Kokkos::Tuple<Real, 2> velocity_exact(const PtType& x);
...
int main(int argc, char* argv[]) {
  ...
  {  // Kokkos scope
    ...
    const Real eps_multiplier =
        input.get_option("mesh_size_multiplier").get_real();
    const Real vel_eps = plane->mesh.appx_mesh_size() * eps_multiplier;
    const Real pse_eps = pse::PSEKernel<PlaneGeometry>::get_epsilon(
        plane->mesh.appx_mesh_size(),
        input.get_option("pse_kernel_width_power").get_real());
    ...
    */
    scalar_view_type psi_exact_passive("psi_exact",
                                       plane->mesh.n_vertices_host());
    scalar_view_type psi_exact_active("psi_exact", plane->mesh.n_faces_host());
    scalar_view_type psi_error_passive("psi_error",
                                       plane->mesh.n_vertices_host());
    scalar_view_type psi_error_active("psi_error", plane->mesh.n_faces_host());
    scalar_view_type phi_exact_passive("phi_exact",
                                       plane->mesh.n_vertices_host());
    scalar_view_type phi_exact_active("phi_exact", plane->mesh.n_faces_host());
    scalar_view_type phi_error_passive("phi_error",
                                       plane->mesh.n_vertices_host());
    scalar_view_type phi_error_active("phi_error", plane->mesh.n_faces_host());
    Kokkos::View<Real* [2]> velocity_exact_passive(
        "velocity_exact", plane->mesh.n_vertices_host());
    scalar_view_type double_dot_exact_passive("double_dot_exact",
                                              plane->mesh.n_vertices_host());
    Kokkos::View<Real* [2]> velocity_exact_active("velocity_exact",
                                                  plane->mesh.n_faces_host());
    scalar_view_type double_dot_exact_active("double_dot_exact",
                                             plane->mesh.n_faces_host());

    Kokkos::parallel_for(
        plane->mesh.n_vertices_host(),
        PlanarGaussianTestVelocity(
            velocity_exact_passive, double_dot_exact_passive,
            plane->mesh.vertices.phys_crds.view, vorticity, divergence));
    Kokkos::parallel_for(
        plane->mesh.n_faces_host(),
        PlanarGaussianTestVelocity(
            velocity_exact_active, double_dot_exact_active,
            plane->mesh.faces.phys_crds.view, vorticity, divergence));
    auto crds = plane->mesh.vertices.phys_crds.view;
    Kokkos::parallel_for(
        plane->mesh.n_vertices_host(), KOKKOS_LAMBDA(const Index i) {
    ...
    Kokkos::parallel_for(
        plane->mesh.n_faces_host(), KOKKOS_LAMBDA(const Index i) {
          const auto xi       = Kokkos::subview(crds, i, Kokkos::ALL);
    ...
    */
    Kokkos::View<Real* [2]> velocity_error_passive(
        "velocity_error", plane->mesh.n_vertices_host());
    compute_error(velocity_error_passive, plane->velocity_passive.view,
                  velocity_exact_passive);

    Kokkos::View<Real* [2]> velocity_error_active("velocity_error",
                                                  plane->mesh.n_faces_host());
    ...
    scalar_view_type ddot_error_passive("double_dot_error",
                                        plane->mesh.n_vertices_host());
    scalar_view_type ddot_error_active("double_dot_error",
                                       plane->mesh.n_faces_host());
    compute_error(ddot_error_passive, plane->double_dot_passive.view,
                  double_dot_exact_passive);
    ErrNorms ddot_err(ddot_error_active, plane->double_dot_active.view,
                      double_dot_exact_active, plane->mesh.faces.area);
    logger.info("double dot error: {}", ddot_err.info_string());

    compute_error(psi_error_passive, plane->stream_fn_passive.view,
                  psi_exact_passive);
    ErrNorms stream_err(psi_error_active, plane->stream_fn_active.view,
                        psi_exact_active, plane->mesh.faces.area);
    logger.info("stream function error: {}", stream_err.info_string());

    compute_error(phi_error_passive, plane->potential_passive.view,
                  phi_exact_passive);
    ErrNorms potential_err(phi_error_active, plane->potential_active.view,
                           phi_exact_active, plane->mesh.faces.area);
    logger.info("potential function error: {}", potential_err.info_string());

#ifdef LPM_USE_VTK
    const std::string resolution_str =
        std::to_string(input.get_option("tree_depth").get_int());
    const std::string eps_str = "eps" + float_str(vel_eps);
    const std::string vtk_file_root =
        input.get_option("output_file_root").get_str() + "_" +
        seed_type::id_string() + resolution_str + eps_str + "_";
    {
      plane->update_host();
    ...
    // setup time stepper
    auto solver =
        std::make_unique<SWERK4<seed_type, topography_type>>(dt, *plane, topo);
    ...
    for (int t_idx = 0; t_idx < nsteps; ++t_idx) {
      ...
      if ((t_idx + 1) % write_frequency == 0) {
        ...
        auto ctr_str                = zero_fill_str(++frame_counter);
        const std::string vtk_fname = vtk_file_root + ctr_str + vtp_suffix();
        logger.info("writing output at t = {} to file: {}", plane->t,
                    vtk_fname);
        vtk.write(vtk_fname);
      }
#endif
...
void input_init(user::Input& input) {
  ...
  user::Option f_coriolis_option("f-coriolis", "-f", "--f-coriolis",
                                 "f coriolis", 0.0);
  input.add_option(f_coriolis_option);

  user::Option beta_coriolis_option("beta-coriolis", "-b", "--beta-coriolis",
                                    "beta coriolis", 0.0);
  input.add_option(beta_coriolis_option);

  user::Option h0_option("h0", "-h0", "--init-depth", "initial uniform depth",
                         1.0);
  input.add_option(h0_option);

  user::Option zeta0_option("zeta0", "-zeta0", "--init-zeta-strength",
                            "initial vorticity strength", 1.0);
  input.add_option(zeta0_option);

  user::Option zetax_option("zetax", "-zetax", "--init-zeta-x",
                            "initial vorticity center, x-coordinate", 0.0);
  input.add_option(zetax_option);

  user::Option zetay_option("zetay", "-zetay", "--init-zeta-y",
                            "initial vorticity center, y-coordinate", 0.0);
  input.add_option(zetay_option);

  user::Option zetab_option("zetab", "-zetab", "--init-zeta-b",
                            "initial vorticity shape parameter", 0.25);
  input.add_option(zetab_option);

  user::Option sigma0_option("sigma0", "-sigma0", "--init-sigma-strength",
                             "initial divergence strength", 0.1);
  input.add_option(sigma0_option);

  user::Option sigmax_option("sigmax", "-sigmax", "--init-sigma-x",
                             "initial divergence center, x-coordinate", 0.5);
  input.add_option(sigmax_option);

  user::Option sigmay_option("sigmay", "-sigmay", "--init-sigma-y",
                             "initial divergence center, y-coordinate", 0.0);
  input.add_option(sigmay_option);

  user::Option sigmab_option("sigmab", "-sigmab", "--init-sigma-b",
                             "initial divergence shape parameter", 2.0);
  input.add_option(sigmab_option);

  user::Option mesh_radius_option("mesh_radius", "-r", "--radius",
                                  "mesh radius", 10.0);
  ...
  user::Option kernel_smoothing_parameter_option(
      "mesh_size_multiplier", "-eps", "--velocity-epsilon",
      "velocity kernel smoothing parameter mesh size multiplier", 0.0);
  input.add_option(kernel_smoothing_parameter_option);

  user::Option pse_power_option("pse_kernel_width_power", "-pse",
                                "--pse-kernel-width-power",
                                "pse kernel width power", 11.0 / 20);
