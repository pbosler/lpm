...
int main(int argc, char* argv[]) {
  ...
  {  // Kokkos scope
    ...
    */
    ...
    sphere->set_kernel_parameters(
        input.get_option("kernel_smoothing_parameter").get_real(), 0);
    ...
    auto solver = std::make_unique<SWERK2<seed_type, topography_type>>(
        dt, *sphere, topo, gmls_params);
    ...
    const std::string eps_str =
        "eps" +
        float_str(input.get_option("kernel_smoothing_parameter").get_real());
...
void tc2_exact_sol(SWE<SeedType>& swe, const CoriolisSphere& coriolis) {
  ...
  Kokkos::parallel_for(
      swe.mesh.n_vertices_host(), KOKKOS_LAMBDA(const Index i) {
        ...
        slap_exact_view(i) =
            (square(u0) + 2 * Omega * u0) * (2 * sin_lat_sq - cos_lat_sq) / g;

        rhs_view(i) = fz_view(i) + cgu_view(i) - ddot_view(i) - slap_view(i) -
                      SphereGeometry::norm2(ui);

        rhs_exact_view(i) = f_zeta_exact_view(i) + cgu_exact_view(i) -
                            ddot_exact_view(i) - slap_exact_view(i) -
