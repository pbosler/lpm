...
struct Input {
  ...
  std::string case_name;  // test case name, will be the beginning of all output
                          // filenames.
  Int init_mesh_depth;    // initial depth of Lagrangian particle/panel mesh
                          // quadtree
...
int main(int argc, char* argv[]) {
  ...
  {  // Kokkos scope
    ...
    */
    ...
    RossbyHaurwitzR vorticity_fn(
        4 * constants::PI / (rossby_order * (rossby_order + 3)), 1,
        rossby_order);
    ...
    ScalarField<VertexField> vert_rel_vort_error("relative_vorticity_error",
                                                 sphere.mesh.n_vertices_host());
    ScalarField<FaceField> face_rel_vort_error("relative_vorticity_error",
                                               sphere.mesh.n_faces_host());
    compute_vorticity_error(vert_rel_vort_error.view, face_rel_vort_error.view,
                            sphere);
    ...
    int output_ctr               = 0;
    const std::string fname_root = "dfs_bve_rRl" + seed_type::id_string() +
                                   "_nlon" + std::to_string(nlon) + "_dt" +
                                   std::to_string(dt);
...
// output initial data
#ifdef LPM_USE_VTK
    {
      const std::string mesh_vtk_file =
          input.vtk_froot + "_" + zero_fill_str(output_ctr) + ".vtp";
      const std::string grid_vtk_file =
          input.vtk_froot + "_" + zero_fill_str(output_ctr) + ".vts";
    ...
    for (int time_idx = 0; time_idx < nsteps; ++time_idx) {
      ...
      compute_vorticity_error(vert_rel_vort_error.view,
                              face_rel_vort_error.view, sphere);
#ifdef LPM_USE_VTK
      {
        const std::string mesh_vtk_file =
            input.vtk_froot + "_" + zero_fill_str(output_ctr) + ".vtp";
    ...
    Kokkos::View<Real*> rel_vort_faces_exact("rel_vort_faces_exact",
                                             sphere.mesh.n_faces_host());
    Kokkos::parallel_for(
        "compute exact rel_vort on LPM faces", sphere.mesh.n_faces_host(),
        KOKKOS_LAMBDA(const Index i) {
          rel_vort_faces_exact(i) =
              abs_vort_faces(i) - 2 * OMG * xyz_faces(i, 2);
        });

    ErrNorms rel_vort_err(face_rel_vort_error.view, rel_vort_faces_exact,
                          sphere.mesh.faces.area);
    logger.info("At t = {}, active panel error is:\n\t{}", sphere.t,
                rel_vort_err.info_string());
...
std::string Input::usage() const {
  ...
  ss << "DFS BVE Test: This program solves the barotropic vorticity equation\n"
     ...
     << "for vorticity advection and a double Fourier series on a mesh to "
        "solve\n"
     << "for velocity.\n";
  ss << "\t" << "optional arguments:\n";
  ss << "\t\t"
     << "-d [--depth] initial depth of the Lagrangian panel quadtree\n"
     ...
     << "\t\t"
     << "-f [--output-frequency] number of time steps between each output "
        "step\n"
