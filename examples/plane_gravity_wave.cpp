...
int main(int argc, char* argv[]) {
  ...
  {  // Kokkos scope
    user::Input input("plane_gravity_wave");
    {
      ...
      user::Option beta_coriolis_option(
          "beta-coriolis", "-b", "--beta-coriolis", "beta coriolis", 0.0);
    ...
    plane->set_kernel_parameters(
        input.get_option("kernel_smoothing_parameter").get_real(),
        pse::PSEKernel<PlaneGeometry>::get_epsilon(
    ...
    const std::string vtk_file_root =
        ...
        seed_type::id_string() + resolution_str + "_";
    {
      plane->update_host();
