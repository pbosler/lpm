...
void exact_velocity(Kokkos::View<Real* [3]> passive_exact,
                    Kokkos::View<Real* [3]> active_exact,
                    Kokkos::View<Real* [3]> passive_error,
                    Kokkos::View<Real* [3]> active_error,
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    */
    ...
    PolyMeshParameters<seed_type> mesh_params(
        input.get_option("tree_depth").get_int(), sphere_radius, amr_buffer,
        amr_limit);
    ...
    auto sphere = std::make_unique<Incompressible2D<seed_type>>(
        mesh_params, coriolis, velocity_eps);
    sphere->init_vorticity(vorticity);

    if (amr) {
      Refinement<seed_type> refiner(sphere->mesh);
      ScalarIntegralFlag max_circulation_flag(
          refiner.flags, sphere->rel_vort_active.view, sphere->mesh.faces.area,
          sphere->mesh.faces.mask, sphere->mesh.n_faces_host(), max_circ_tol);

      max_circulation_flag.set_tol_from_relative_value();
      max_circ_tol = max_circulation_flag.tol;

      logger.info("amr is enabled with limit {}, max_circ_tol = {}", amr_limit,
                  max_circ_tol);
    ...
    Kokkos::View<Real* [3]> velocity_exact_passive("velocity_exact",
                                                   mesh_params.nmaxverts);
    Kokkos::View<Real* [3]> velocity_exact_active("velocity_exact",
                                                  mesh_params.nmaxfaces);
    Kokkos::View<Real* [3]> vel_error_passive("vel_error",
                                              mesh_params.nmaxverts);
    Kokkos::View<Real* [3]> vel_error_active("vel_error",
                                             mesh_params.nmaxfaces);
    ...
    gmls::Params gmls_params(
        input.get_option("remesh_interpolation_order").get_int());
    ...
    ***************************************************/
    ...
    for (int t_idx = 0; t_idx < nsteps; ++t_idx) {
      ...
      if (do_remesh) {
        ...
        auto new_sphere = std::make_unique<Incompressible2D<seed_type>>(
            mesh_params, coriolis,
        ...
        if (amr) {
          ...
          ScalarIntegralFlag max_circulation_flag(
              ...
              new_sphere->mesh.n_faces_host(), max_circ_tol);
        ...
        exact_vorticity(new_sphere->tracer_passive.at("exact_vorticity").view,
                        new_sphere->tracer_active.at("exact_vorticity").view,
                        vorticity, *new_sphere);

        sphere = std::move(new_sphere);
        solver.reset(
            new Incompressible2DRK2<seed_type>(dt, *sphere, solver->t_idx));
      }
      ...
      exact_vorticity(sphere->tracer_passive.at("exact_vorticity").view,
                      sphere->tracer_active.at("exact_vorticity").view,
                      vorticity, *sphere);
      compute_error(sphere->tracer_passive.at("vorticity_error").view,
                    sphere->rel_vort_passive.view,
                    sphere->tracer_passive.at("exact_vorticity").view);
      ...
      logger.info(
          "t = {}: relvort err (l1, l2, linf) = ({}, {}, {}), vel_err = ({}, "
          "{}, {})",
          sphere->t, zeta_err.l1, zeta_err.l2, zeta_err.linf, vel_err.l1,
      ...
      if ((t_idx + 1) % write_frequency == 0) {
        ...
        auto ctr_str                = zero_fill_str(++frame_counter);
        const std::string vtk_fname = vtk_file_root + ctr_str + vtp_suffix();
        logger.info("writing output at t = {} to file: {}", sphere->t,
                    vtk_fname);
        vtk.write(vtk_fname);
      }
#endif
...
void exact_velocity(Kokkos::View<Real* [3]> passive_exact,
  ...
  Kokkos::parallel_for(
      sphere.mesh.n_vertices_host(), KOKKOS_LAMBDA(const Index i) {
  ...
  Kokkos::parallel_for(
      sphere.mesh.n_faces_host(), KOKKOS_LAMBDA(const Index i) {
...
void init_input(user::Input& input) {
  ...
  user::Option remesh_interpolation_order(
      "remesh_interpolation_order", "-ro", "--remesh-order",
      "polynomial order for gmls-based remesh interpolation", 4);
