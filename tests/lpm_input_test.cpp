...
TEST_CASE("input", "") {
  ...
  SECTION("Option tests") {
    ...
    Option o3("str_opt", "-s", "--string", "a string", std::string("i am a string"));
    Option o4("bool_opt", "-t", "--true", "a bool", false);
    const std::set<std::string> str_vals({"val1", "val2"});
    Option o5("str_opts", "-ss", "--string-set", "string_set", std::string("val1"), str_vals);
  ...
  SECTION("Input tests") {
    ...
    Option input_file_option("input_file", "-i", "--input", "input file name", std::string("test_file_in.txt"));
    Option output_file_option("output_file", "-o", "--output", "output file name", std::string("test_file_out.txt"));
    Option tfinal_option("tfinal", "-tf", "--time_final", "time final", 0.1);
    Option nsteps_option("nsteps", "-n", "--nsteps", "number of steps", 10);
    Option tree_depth_option("tree_depth", "-d", "--depth", "mesh tree depth", 3);
