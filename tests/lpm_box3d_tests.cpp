...
TEST_CASE("box3d", "[tree]") {
  ...
  SECTION("unit tests") {
    ...
    logger.info("box0.aspect_ratio() = {}. Expected result: 1",
                box0.aspect_ratio());
    ...
    logger.info("box0.centroid() = [{}, {}, {}]. Expected result: [0,0,0]",
                c00[0], c00[1], c00[2]);
    REQUIRE(FloatingPoint<Real>::zero(
        SphereGeometry::square_euclidean_distance(c0, c00)));
    REQUIRE(FloatingPoint<Real>::zero(
        SphereGeometry::square_euclidean_distance(c0, origin)));
    ...
    logger.info(
        "box0.closest_pt_l1([2,0,0]) = [{}, {}, {}]. Expected result [1,0,0]",
        cp[0], cp[1], cp[2]);
