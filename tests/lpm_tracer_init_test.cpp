...
TEST_CASE("planar particles", "") {
  Comm comm;
  Logger<> logger("lpm_tracer_init::planar_particles_test", Log::level::info,
                  comm);
}
...
TEST_CASE("planar mesh", "") {
  ...
  SECTION("triangular panels") {
    ...
    tracer_verts.emplace(ph.name(),
                         ScalarField<VertexField>(ph.name(), params.nmaxverts));
    tracer_verts.emplace(pd.name(),
                         ScalarField<VertexField>(pd.name(), params.nmaxverts));
    tracer_verts.emplace(pc.name(),
                         ScalarField<VertexField>(pc.name(), params.nmaxverts));
    tracer_verts.emplace("sum_all",
                         ScalarField<VertexField>("sum_all", params.nmaxverts));

    tracer_faces.emplace(ph.name(),
                         ScalarField<FaceField>(ph.name(), params.nmaxfaces));
    tracer_faces.emplace(pd.name(),
                         ScalarField<FaceField>(pd.name(), params.nmaxfaces));
    tracer_faces.emplace(pc.name(),
                         ScalarField<FaceField>(pc.name(), params.nmaxfaces));
    tracer_faces.emplace("sum_all",
                         ScalarField<FaceField>("sum_all", params.nmaxfaces));
...
TEST_CASE("spherical particles", "") {
  Comm comm;
  Logger<> logger("lpm_tracer_init::spherical_particles_test", Log::level::info,
                  comm);
}

TEST_CASE("spherical mesh", "") {
  Comm comm;
  Logger<> logger("lpm_tracer_init::spherical_mesh_test", Log::level::debug,
                  comm);
  ...
  SECTION("triangular panels") {
    ...
    tracer_verts.emplace(gh.name(),
                         ScalarField<VertexField>(gh.name(), params.nmaxverts));
    tracer_verts.emplace(cb.name(),
                         ScalarField<VertexField>(cb.name(), params.nmaxverts));
    tracer_verts.emplace(sc.name(),
                         ScalarField<VertexField>(sc.name(), params.nmaxverts));
    tracer_verts.emplace(mv.name(),
                         ScalarField<VertexField>(mv.name(), params.nmaxverts));
    tracer_verts.emplace(lt.name(),
                         ScalarField<VertexField>(lt.name(), params.nmaxverts));

    tracer_faces.emplace(gh.name(),
                         ScalarField<FaceField>(gh.name(), params.nmaxfaces));
    tracer_faces.emplace(cb.name(),
                         ScalarField<FaceField>(cb.name(), params.nmaxfaces));
    tracer_faces.emplace(sc.name(),
                         ScalarField<FaceField>(sc.name(), params.nmaxfaces));
    tracer_faces.emplace(mv.name(),
                         ScalarField<FaceField>(mv.name(), params.nmaxfaces));
    tracer_faces.emplace(lt.name(),
                         ScalarField<FaceField>(lt.name(), params.nmaxfaces));
    ...
    for (Index i = 0; i < pm.vertices.nh(); ++i) {
      if (!(FloatingPoint<Real>::equiv(schview(i), 0.1) or
            FloatingPoint<Real>::equiv(schview(i), 1))) {
    ...
    Kokkos::parallel_reduce(
        ...
        Kokkos::MinMax<Real>(sc_minmax_verts));
    logger.debug("sc_minmax = ({}, {})", sc_minmax_verts.min_val,
                 sc_minmax_verts.max_val);
