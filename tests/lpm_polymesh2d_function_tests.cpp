...
struct PlanePolyMeshFnUnitTest {
  ...
  void run() {
...

    {
      ...
      PolyMeshParameters<QuadRectSeed> qr0_params(zero_depth, mesh_radius,
                                                  amr_depth);
      ...
      Kokkos::parallel_for(
          8, KOKKOS_LAMBDA(const Index i) {
            if (i == 0) {
              ...
              fidx0() = qr0.locate_pt_walk_search(qp, 2);
            } else if (i == 4) {
              ridx() = qr0.nearest_root_face(qp);
            } else if (i == 5) {
              fidx1() = qr0.locate_pt_tree_search(qp, 0);
            } else if (i == 6) {
              fidx2() = qr0.locate_face_containing_pt(qp);
            } else if (i == 7) {
      ...
      Kokkos::parallel_for(
          qr0.n_faces_host(), KOKKOS_LAMBDA(const Index i) {
      ...
      Kokkos::parallel_for(
          qr0.n_vertices_host(), KOKKOS_LAMBDA(const Index i) {
      ...
      logger.info("parent edge 0 has {} leaves: [{}, {}, {}]",
                  n_leaf_edges0_host(), leaf_edges0_host[0],
                  leaf_edges0_host[1], leaf_edges0_host[2]);
...
struct InterpolationTest {
  ...
  InterpolationTest(const int sd, const int ed, const int n_unif = 60)
      : start_depth(sd),
        end_depth(ed),
        radius((std::is_same<typename SeedType::geo, PlaneGeometry>::value
                    ? 6
                    : 1)),
        grid(n_unif, radius) {}
  ...
  void run() {
    ...
    for (int i = 0; i < (end_depth - start_depth) + 1; ++i) {
      ...
      ScalarField<VertexField> tracer_verts_interp("tracer_interp",
                                                   pm.vertices.nh());
      ScalarField<FaceField> tracer_faces_interp("tracer_interp",
                                                 pm.faces.nh());
      ...
      Kokkos::parallel_for(
          pm.vertices.nh(), KOKKOS_LAMBDA(const Index i) {
            const auto xy = Kokkos::subview(vcrds, i, Kokkos::ALL);
            vg(i)         = tracer(xy);
          });
      Kokkos::parallel_for(
          pm.faces.nh(), KOKKOS_LAMBDA(const Index i) {
      ...
      pm.scalar_interpolate(grid_tracer_interp, grid.pts, tracer_verts);
      ...
      ErrNorms face_err_norms(face_error, tracer_faces_interp.view,
                              tracer_faces.view, pm.faces.area_host());
      ...
      Kokkos::parallel_for(
          pm.n_vertices_host(),
          ComputeErrorFtor<scalar_view_type, scalar_view_type, scalar_view_type,
                           1>(vert_error, tracer_verts_interp.view,
                              tracer_verts.view));
      ;
      ...
      logger.info("polymesh faces interpolation error: {}",
                  face_err_norms.info_string());
    ...
    REQUIRE((face_interp_l2_rate.back() > 2.0 or
             face_interp_l2_rate.back() == Approx(2.0).epsilon(0.12)));
  }
};
...
TEST_CASE("interpolation_test", "") {
  ...
  SECTION("planar tri") {
    ...
    InterpolationTest<seed_type, tracer_type, grid_type> interp_test(
  ...
  //
  //     InterpolationTest<seed_type, tracer_type, grid_type>
  //     interp_test(start_depth, end_depth, 45); interp_test.run();
  //   }
  SECTION("planar quad") {
...
TEST_CASE("mesh to mesh", "") {
  ...
  SECTION("sphere") {
    ...
    ScalarField<VertexField> ic_tracer_verts_interp("tracer_interp",
                                                    ic.vertices.nh());
    ScalarField<FaceField> ic_tracer_faces_interp("tracer_interp",
                                                  ic.faces.nh());
    ...
    ScalarField<VertexField> cs_tracer_verts_interp("tracer_interp",
                                                    cs.vertices.nh());
    ScalarField<FaceField> cs_tracer_faces_interp("tracer_interp",
                                                  cs.faces.nh());
    ...
    Kokkos::parallel_for(
        ic.vertices.nh(), KOKKOS_LAMBDA(const Index i) {
          const auto xy = Kokkos::subview(ic_vcrds, i, Kokkos::ALL);
          ic_vt(i)      = tracer(xy);
        });
    Kokkos::parallel_for(
        ic.faces.nh(), KOKKOS_LAMBDA(const Index i) {
          const auto xy = Kokkos::subview(ic_fcrds, i, Kokkos::ALL);
          ic_ft(i)      = tracer(xy);
        });

    Kokkos::parallel_for(
        cs.vertices.nh(), KOKKOS_LAMBDA(const Index i) {
          const auto xy = Kokkos::subview(cs_vcrds, i, Kokkos::ALL);
          cs_vt(i)      = tracer(xy);
        });
    Kokkos::parallel_for(
        cs.faces.nh(), KOKKOS_LAMBDA(const Index i) {
          const auto xy = Kokkos::subview(cs_fcrds, i, Kokkos::ALL);
          cs_ft(i)      = tracer(xy);
        });

    ic.scalar_interpolate(cs_tracer_verts_interp.view, cs_vcrds,
                          ic_tracer_verts);
    ic.scalar_interpolate(cs_tracer_faces_interp.view, cs_fcrds,
                          ic_tracer_verts);
    cs.scalar_interpolate(ic_tracer_verts_interp.view, ic_vcrds,
                          cs_tracer_verts);
    cs.scalar_interpolate(ic_tracer_faces_interp.view, ic_fcrds,
                          cs_tracer_verts);

    Kokkos::parallel_for(
        ic.n_vertices_host(),
        ComputeErrorFtor<scalar_view_type, scalar_view_type, scalar_view_type,
                         1>(ic_vert_error, ic_tracer_verts_interp.view,
                            ic_tracer_verts.view));
    Kokkos::parallel_for(
        cs.n_vertices_host(),
        ComputeErrorFtor<scalar_view_type, scalar_view_type, scalar_view_type,
                         1>(cs_vert_error, cs_tracer_verts_interp.view,
                            cs_tracer_verts.view));
