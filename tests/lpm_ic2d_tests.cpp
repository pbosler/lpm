...
template <typename SeedType, typename SolverType> struct TimeConvergenceTest {
  using geo = typename SeedType::geo;
  using Coriolis = typename std::conditional<std::is_same<
    geo, PlaneGeometry>::value, CoriolisBetaPlane, CoriolisSphere>::type;
  using Vorticity = typename std::conditional<std::is_same<
    geo, PlaneGeometry>::value, CollidingDipolePairPlane, GaussianVortexSphere>::type;
  ...
  TimeConvergenceTest(const int depth, const Real r, const Real tf,
    const std::vector<Real>& epss, const std::vector<int>& nsteps,
  ...
  void run() {
    ...
    for (int i=0; i<nsteps.size()-1; ++i) {
      ...
      ic2d = std::make_unique<Incompressible2D<SeedType>>(mesh_params, coriolis, eps);
      ...
      auto face_err_norms = ErrNorms(face_position_error, pos, ref_pos, ic2d->mesh.faces.area);
      logger.info("ns = {}, l1 = {}, l2 = {}, linf = {}", ns,
        face_err_norms.l1, face_err_norms.l2, face_err_norms.linf);
  ...
  private:
    void setup() {
      ...
      reference_sol = std::make_unique<Incompressible2D<SeedType>>(mesh_params, coriolis, eps);
      Vorticity vorticity;
      reference_sol->init_vorticity(vorticity);
      reference_sol->init_direct_sums();
      const auto vel_range = reference_sol->velocity_active.range(reference_sol->mesh.n_faces_host());
      ...
      face_position_error = CoordView("face_position_error", reference_sol->mesh.n_faces_host());
      logger.info("setup complete for {}\n\tsetup time: {} seconds", SeedType::id_string(), toc(setup_start));
    }
};
...
TEST_CASE("sphere", "[conv]") {
  ...
  const std::vector<int> nsteps = {30, 60, 120, 240};
...
//   }
  ...
  SECTION("quadrilateral panels") {
    ...
    TimeConvergenceTest<seed_type, Solver> test_case(tree_depth, sphere_radius,
      tfinal, eps, nsteps, case_name);
