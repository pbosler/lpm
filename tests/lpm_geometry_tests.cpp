...
TEST_CASE("lpm_geometry", "") {
  ...
  SECTION("planar tests") {
    ...
    ko::View<Real>::HostMirror host_scalar =
        ko::create_mirror_view(scalar_result);
    ko::View<Real[2], Dev> result("res");

    ko::View<Real[2], Dev>::HostMirror host_result =
        ko::create_mirror_view(result);
    ...
    ko::parallel_for(
        1, KOKKOS_LAMBDA(int i) {
          ...
          scalar_result() = PlaneGeometry::tri_area<ko::View<Real[2], Dev>>(
              r2device_a, r2device_b, r2device_c);
          ...
          LPM_KERNEL_REQUIRE(
              FloatingPoint<Real>::equiv(PlaneGeometry::mag(r2device_a), 1));

          // test midpoint on device
          PlaneGeometry::midpoint(result, ko::subview(vecs, 0, ko::ALL),
          ...
          // test distance
          scalar_result() = PlaneGeometry::distance(
              ko::subview(vecs, 0, ko::ALL), ko::subview(vecs, 2, ko::ALL));
          LPM_KERNEL_REQUIRE(
              FloatingPoint<Real>::equiv(scalar_result(), 2 * sqrt(26.0)));
    ...
    ko::parallel_for(
        1, KOKKOS_LAMBDA(int i) {
          ...
          printf("res() = %f, pi/3 = %f, diff = %g, tol = %g\n", res(),
                 constants::PI / 3, abs(res() - constants::PI / 3),
                 constants::ZERO_TOL);

          LPM_KERNEL_REQUIRE(FloatingPoint<Real>::equiv(
              res(), constants::PI / 3.0, constants::ZERO_TOL));
          ...
          LPM_KERNEL_REQUIRE(
              FloatingPoint<Real>::equiv(vres(0), 0.5 * sqrt(2.0)));

          SphereGeometry::cross(vres, a, b);
          LPM_KERNEL_REQUIRE(
              (FloatingPoint<Real>::equiv(vres(0), 2.0 / 3.0,
                                          constants::ZERO_TOL) and
               FloatingPoint<Real>::equiv(vres(1), 2.0 / 3.0,
                                          constants::ZERO_TOL)) and
          ...
          LPM_KERNEL_REQUIRE(FloatingPoint<Real>::equiv(
              res(), 2 * constants::PI / 3, constants::ZERO_TOL));
        });
  ...
  SECTION("circle tests") {  // circle tests
    ...
    printf("endpt0 = (%f, %f) has r = %f\n", r2a[0], r2a[1],
           CircularPlaneGeometry::mag(r2a));
    printf("endpt1 = (%f, %f) has r = %f\n", r2d[0], r2d[1],
           CircularPlaneGeometry::mag(r2d));
    printf("midpt = (%f, %f) has r = %f\n", cmid[0], cmid[1],
           CircularPlaneGeometry::mag(cmid));
    REQUIRE(FloatingPoint<Real>::equiv(CircularPlaneGeometry::mag(cmid), 1.0));

    printf("theta_b = %f\n", CircularPlaneGeometry::theta(r2b));
    REQUIRE(FloatingPoint<Real>::equiv(CircularPlaneGeometry::theta(r2b),
                                       0.5 * constants::PI));

    printf("dtheta(b,c) = %f\n", CircularPlaneGeometry::dtheta(r2b, r2c));
    REQUIRE(FloatingPoint<Real>::equiv(CircularPlaneGeometry::dtheta(r2a, r2c),
                                       0.5 * constants::PI));

    const Real qsa = CircularPlaneGeometry::quad_sector_area(r2a, r2c);
    printf("area(a,b,c,d) = %f\n", qsa);
    REQUIRE(FloatingPoint<Real>::equiv(
        qsa, (0.5 * (constants::PI / 2) * (square(1.0) - square(0.5)))));
    REQUIRE(FloatingPoint<Real>::equiv(4 * qsa + constants::PI * square(0.5),
                                       constants::PI));
    ...
    const Real ctr[2] = {std::cos(constants::PI / 4) * 3 / 4,
                         std::sin(constants::PI / 4) * 3 / 4};
    ...
    Real exbc[2] = {0.75 * std::cos(constants::PI / 4),
                    0.75 * std::sin(constants::PI / 4)};
