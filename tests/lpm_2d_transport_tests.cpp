...
struct TimeConvergenceTest {
  ...
  typedef typename std::conditional<
      std::is_same<typename SeedType::geo, PlaneGeometry>::value, PlanarHump,
      SphericalGaussianHills>::type TracerType1;
  typedef typename std::conditional<
      std::is_same<typename SeedType::geo, PlaneGeometry>::value, PlanarCone,
      SphericalCosineBells>::type TracerType2;
  typedef typename std::conditional<
  ...
  TimeConvergenceTest(const int tl, const int amr, const Real r, const Real tf,
      ...
      : tree_lev(tl), amr_limit(amr), radius(r), tfinal(tf), nsteps(ns) {
  ...
  void run() {
    ...
    for (int n_idx = 0; n_idx < nsteps.size(); ++n_idx) {
      ...
      int frame_counter = 0;
      std::stringstream ss;
      ss << "transport_2d_" << SeedType::id_string() << tree_lev << "_dt_conv_"
         << dt_str(dt);
      const auto base_filename = ss.str();
      ss.str("");
      {
        VtkPolymeshInterface<SeedType> vtk = vtk_interface(*tm);
        vtk.write(base_filename + zero_fill_str(frame_counter++) +
                  vtp_suffix());
        ss.str("");
      }
#endif
      ...
      Kokkos::View<Real**> face_position_error(
          "face_position_error", tm->mesh.n_faces_host(), SeedType::geo::ndim);
      auto final_pos = tm->mesh.faces.phys_crds.view;
      auto init_pos  = tm->mesh.faces.lag_crds.view;

      auto face_err_norms = ErrNorms(face_position_error, final_pos, init_pos,
                                     tm->mesh.faces.area);
...
TEST_CASE("planar meshes", "[conv]") {
  ...
  SECTION("triangular panels") {
    ...
    TimeConvergenceTest<velocity_field, seed_type> test_case(
        tree_lev, amr_limit, radius, tfinal, nsteps);
    test_case.run();
  }
