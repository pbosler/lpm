...
TEST_CASE("lat_lon_unit_tests", "") {
  ...
  SECTION("constructor 1") {
    ...
    for (int i = 0; i < nlats.size(); ++i) {
      ...
      Kokkos::parallel_reduce(
          npts, KOKKOS_LAMBDA(const Index& i, Real& a) { a += ll.wts(i); },
          surf_area);

      logger.info(
          "ll resolution {} surface area error = {}",
          ll.dlambda * constants::RAD2DEG,
          std::abs(4 * constants::PI - surf_area) / (4 * constants::PI));
  ...
  SECTION("constructor 2") {
    ...
    for (int i = 0; i < nunif.size(); ++i) {
      ...
      Real surf_area = 0;
      Kokkos::parallel_reduce(
          ll.size(), KOKKOS_LAMBDA(const Index& i, Real& a) { a += ll.wts(i); },
          surf_area);
