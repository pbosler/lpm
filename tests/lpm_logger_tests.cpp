...
TEST_CASE("basic logger", "[logging]") {
  ...
  SECTION("console only, default comm") {
    ...
    Logger<LogNoFile> mylog("ekat_log_test_console_only", Log::level::debug,
                            comm);
  ...
  SECTION("console and file logging, with mpi rank info") {
    Logger<LogBasicFile<Log::level::trace>> mylog("combined_console_file_mpi",
                                                  Log::level::debug, comm);

    mylog.debug(
        "here is a debug message that will also show up in this rank's log "
        "file.");

    // the file level is trace, but the log level is debug (debug > trace);
    // trace messages will be skipped.
    ...
    const std::string logfilename =
        "combined_console_file_mpi_rank0_logfile.txt";
    std::ifstream lf(logfilename);
    REQUIRE(lf.is_open());
  ...
  SECTION("Debug excludes Trace") {
    Logger<LogBasicFile<Log::level::debug>> mylog("debug_excludes_trace",
                                                  Log::level::debug, comm);

    mylog.debug(
        "this debug message will show up in both the console and the log "
        "file.");
    mylog.trace(
        "Entered 'debug excludes tracer' section; this message will not show "
        "up anywhere.");
  }

  SECTION("log file check") {
    // The log file is not necessarily written as you write log messages.
    // For performance reasons, log messages may first be collated into a
    // buffer; that buffer is then written according to spdlog's internal logic.
    // This means that in order to check the previous section's log file, we
    // have to wait for its log to go out of scope (which guarantees that the
...
#include "spdlog/spdlog.h"
    ...
    mylog.warn(
        "This is a test.  Here is a warning message.  This is only a test.");
  }
}
