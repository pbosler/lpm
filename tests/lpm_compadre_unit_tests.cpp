...
TEST_CASE("compadre_unit_tests", "") {
  ...
  SECTION("plane geometry") {
    ...
    Kokkos::parallel_for(
        quad_plane.n_vertices_host(), KOKKOS_LAMBDA(const Index i) {
          gaussian_verts.view(i) =
              gaussian(Kokkos::subview(vcrds, i, Kokkos::ALL));
        });
    Kokkos::parallel_for(
        quad_plane.n_faces_host(), KOKKOS_LAMBDA(const Index i) {
          gaussian_faces.view(i) =
              gaussian(Kokkos::subview(fcrds, i, Kokkos::ALL));
        });
    ...
    gmls::Neighborhoods mesh_neighbors(
        h_src_crds, quad_plane.vertices.phys_crds.get_host_crd_view(),
        gmls_params);
    ...
    Kokkos::View<Real*> grid_lap_gaussian("laplacian_gauss_exact",
                                          grid.pts.extent(0));
    Kokkos::parallel_for(
        grid.pts.extent(0), KOKKOS_LAMBDA(const Index i) {
    ...
    Kokkos::View<Real*> gauss_lap_err("laplacian_gauss_err",
                                      grid.pts.extent(0));

    ErrNorms gauss_err_norms(gauss_err, gaussian_gmls, grid_gaussian, grid.wts);
    ErrNorms lap_gauss_err_norms(gauss_lap_err, lap_gaussian_gmls,
                                 grid_lap_gaussian, grid.wts);
  ...
  SECTION("sphere geometry") {
    ...
    Kokkos::parallel_for(
        "init rh54 verts", trisphere.n_vertices_host(),
    ...
    Kokkos::parallel_for(
        "init rh54 faces", trisphere.n_faces_host(),
        KOKKOS_LAMBDA(const Index i) {
    ...
    Kokkos::parallel_for(
        ll.n(), KOKKOS_LAMBDA(const Index i) {
    ...
    auto scalar_gmls = gmls::sphere_scalar_gmls(
        src_crds, ll.pts, face_neighbors, gmls_params, gmls_ops);
    ...
    ErrNorms lap_rh54_err_norms(lap_rh54_error, lap_rh54_gmls, ll_lap_rh54,
                                ll.wts);
