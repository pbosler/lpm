...
struct PSEConvergenceTest {
  ...
  void run() {
    ...
    for (int i = 0; i < (end_depth - start_depth) + 1; ++i) {
      ...
      const auto pm = std::unique_ptr<TransportMesh2d<SeedType>>(
          new TransportMesh2d<SeedType>(params));
      ...
      Kokkos::parallel_for(
          pm->mesh.vertices.nh(), KOKKOS_LAMBDA(const Index i) {
      ...
      Kokkos::parallel_for(
          pm->mesh.faces.nh(), KOKKOS_LAMBDA(const Index i) {
            const auto xy = Kokkos::subview(flxy, i, Kokkos::ALL);
            rlap_faces(i) = tracer.laplacian(xy);
          });

      Kokkos::TeamPolicy<> vertex_policy(pm->mesh.vertices.nh(),
                                         Kokkos::AUTO());
      ...
      Kokkos::parallel_for(vertex_policy,
                           pse::ScalarInterpolation<pse_type>(
                               verts_pse_interp, vlxy, flxy, faces_tracer,
                               face_area, pse_epsilon, pm->mesh.faces.nh()));
      Kokkos::parallel_for(
          vertex_policy,
          pse::ScalarLaplacian<pse_type>(verts_pse_lap, vlxy, flxy,
                                         verts_tracer, faces_tracer, face_area,
                                         pse_epsilon, pm->mesh.faces.nh()));
      Kokkos::parallel_for(face_policy,
                           pse::ScalarInterpolation<pse_type>(
                               faces_pse_interp, flxy, flxy, faces_tracer,
                               face_area, pse_epsilon, pm->mesh.faces.nh()));
      Kokkos::parallel_for(
          face_policy,
          pse::ScalarLaplacian<pse_type>(faces_pse_lap, flxy, flxy,
                                         faces_tracer, faces_tracer, face_area,
                                         pse_epsilon, pm->mesh.faces.nh()));
      ...
      ErrNorms interp_err(face_interp_error, faces_pse_interp, faces_tracer,
                          face_area);
      ...
      const auto erinterp =
          reduce_error(face_interp_error, faces_pse_interp, faces_tracer);
      const auto erlap =
          reduce_error(face_lap_error, faces_pse_lap, rlap_faces);
    ...
    logger.info(
        convergence_table("dx", dxs, "interp_l1", interp_l1, interp_l1_rate));
    logger.info(
        convergence_table("dx", dxs, "interp_l2", interp_l2, interp_l2_rate));
    logger.info(convergence_table("dx", dxs, "interp_linf", interp_linf,
                                  interp_linf_rate));
    ...
    logger.info(
        convergence_table("dx", dxs, "lap_linf", lap_linf, lap_linf_rate));
  }
};
...
TEST_CASE("planar mesh", "") {
  ...
  SECTION("triangular panels") {
    ...
    PSEConvergenceTest<velocity_type, seed_type> pse_test(start_depth,
                                                          end_depth, radius);
    pse_test.run();
  }
