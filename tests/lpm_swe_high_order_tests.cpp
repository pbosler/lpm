...
TEST_CASE("high order kernels", "[swe]") {
  ...
  for (int i = 0; i < n_eps; ++i) {
    logger.info("eps = {}, b2_scaled error info: {}", h_eps_vals(i),
                b2_scaled_err[i].info_string());
    logger.info("eps = {}, b8_scaled error info: {}", h_eps_vals(i),
                b8_scaled_err[i].info_string());
  ...
  for (int i = 0; i < n_eps; ++i) {
    ...
    q2err.push_back(ErrNorms(err_view2, qview2, qtest2, weights));
    q8err.push_back(ErrNorms(err_view8, qview8, qtest8, weights));
  }
  for (int i = 0; i < n_eps; ++i) {
    logger.info("eps = {}, q2 err info: {}", h_eps_vals(i),
                q2err[i].info_string());
    logger.info("eps = {}, q8 err info: {}", h_eps_vals(i),
                q8err[i].info_string());
