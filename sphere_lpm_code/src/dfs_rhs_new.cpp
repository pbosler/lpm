...
namespace SpherePoisson {
    ...
    void vals2CoeffsDbl(GridType grid_type, view_1d<Complex> cn, view_2d<Real> f, view_2d<Complex> F)
    {
        // double up the grid
    ...
    // for solving the equation in the Fourier coefficient space
     void poisson_rhs(GridType grid_type, view_1d<Int>ie, view_1d<Int>io, view_1d<Complex>cn, view_2d<Real> f, view_2d<Complex> rhso, view_2d<Complex> rhse)

    {
        const int nrows = 2*(f.extent(0) - (grid_type == GridType::Shifted ? 0 : 1));
        const int ncols = f.extent(1);
