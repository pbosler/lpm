...
struct BVERK4Update {
  ...
  BVERK4Update(crd_view& x_inout, const crd_view& xs1, const crd_view& xs2,
               const crd_view& xs3, const crd_view& xs4,
               scalar_view_type zeta_inout, const scalar_view_type& zs1,
               const scalar_view_type& zs2, const scalar_view_type& zs3,
               const scalar_view_type& zs4)
      : x(x_inout),
        x1(xs1),
        x2(xs2),
        x3(xs3),
...
void BVERK4::advance_timestep(BVESphere<SeedType>& sph) {
  return advance_timestep(sph.vertices.phys_crds.view, sph.rel_vort_verts.view,
                          sph.velocity_verts.view, sph.faces.phys_crds.view,
                          sph.rel_vort_faces.view, sph.velocity_faces.view,
                          sph.faces.area, sph.faces.mask);
}

void BVERK4::advance_timestep(crd_view& vx, scalar_view_type& vzeta,
                              vec_view& vvel, crd_view& fx,
                              scalar_view_type& fzeta, vec_view& fvel,
                              const scalar_view_type& fa,
  ...
  ko::parallel_for("RK4-1 vertex vorticity", nverts,
                   BVEVorticityTendency(vertvort1, vertvel, dt, Omega));

  KokkosBlas::scal(facex1, dt, facevel);
  ko::parallel_for("RK4-1 face vorticity", nfaces,
                   BVEVorticityTendency(facevort1, facevel, dt, Omega));
  ...
  ko::parallel_for("RK4-2 vertex velocity", vertex_policy,
                   BVEVertexVelocity(vertvel, vertxwork, facexwork,
                                     facevortwork, facearea, facemask, nfaces));
  ko::parallel_for("RK4-2 face velocity", face_policy,
                   BVEFaceVelocity(facevel, facexwork, facevortwork, facearea,
                                   facemask, nfaces));
  KokkosBlas::scal(vertx2, dt, vertvel);
  KokkosBlas::scal(facex2, dt, facevel);
  ko::parallel_for("RK4-2 vertex vorticity", nverts,
                   BVEVorticityTendency(vertvort2, vertvel, dt, Omega));
  ko::parallel_for("RK4-2 face vorticity", nfaces,
                   BVEVorticityTendency(facevort2, facevel, dt, Omega));
  ...
  ko::parallel_for("RK4-3 vertex vorticity", nverts,
                   BVEVorticityTendency(vertvort3, vertvel, dt, Omega));
  ...
  ko::parallel_for("RK4-4 vertex vorticity", nverts,
                   BVEVorticityTendency(vertvort4, vertvel, dt, Omega));
  ko::parallel_for("RK4-4 face vorticity", nfaces,
                   BVEVorticityTendency(facevort4, facevel, dt, Omega));

  ko::parallel_for("RK4 vertex update", nverts,
                   BVERK4Update(vertx, vertx1, vertx2, vertx3, vertx4, vertvort,
                                vertvort1, vertvort2, vertvort3, vertvort4));
  ko::parallel_for("RK4 face update", nfaces,
                   BVERK4Update(facex, facex1, facex2, facex3, facex4, facevort,
                                facevort1, facevort2, facevort4, facevort4));

  ko::parallel_for("RK4-0 vertex velocity", vertex_policy,
                   BVEVertexVelocity(vertvel, vertx, facex, facevort, facearea,
                                     facemask, nfaces));
  ko::parallel_for(
      "RK4-0 face velocity", face_policy,
