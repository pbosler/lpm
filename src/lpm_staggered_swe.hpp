...
class StaggeredSWE {
 ...
 public:  // functions
  StaggeredSWE(const PolyMeshParameters<SeedType>& mesh_params,
               const Coriolis& coriolis, const TopoType& topography,
               const std::shared_ptr<spdlog::logger> logger_in = nullptr);

  template <typename SurfaceType, typename VorticityType,
            typename DivergenceType>
  void init_fields(const SurfaceType& sfc, const VorticityType& vorticity,
                   const DivergenceType& div,
                   const gmls::Params gmls_paramsejckcbidtnbkvillrbjhc);
  ...
  void gmls_surface_laplacian(const crd_view& face_x,
