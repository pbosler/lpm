...
struct RK4Update {
  ...
  RK4Update(Kokkos::View<Real**> x, const Kokkos::View<Real**> k1,
            const Kokkos::View<Real**> k2, const Kokkos::View<Real**> k3,
            const Kokkos::View<Real**> k4, const Real ddt)
      : xcrds(x), x1(k1), x2(k2), x3(k3), x4(k4), dt(ddt) {}

  KOKKOS_INLINE_FUNCTION
  void operator()(const Index i) const {
    for (int j = 0; j < xcrds.extent(1); ++j) {
      xcrds(i, j) +=
          dt * (x1(i, j) + 2 * x2(i, j) + 2 * x3(i, j) + x4(i, j)) / 6;
    }
  }
};
...
void Transport2dRK4<SeedType>::advance_timestep() {
  ...
  Kokkos::parallel_for(
      nverts, VelocityKernel<VelocityFtor>(vertx2, vertxwork, t + 0.5 * dt));
  Kokkos::parallel_for(
      nfaces, VelocityKernel<VelocityFtor>(facex2, facexwork, t + 0.5 * dt));
  // RK Stage 3
  //    setup input
  ...
  //    compute velocity
  Kokkos::parallel_for(
      nverts, VelocityKernel<VelocityFtor>(vertx3, vertxwork, t + 0.5 * dt));
  Kokkos::parallel_for(
      nfaces, VelocityKernel<VelocityFtor>(facex3, facexwork, t + 0.5 * dt));
  //  RK Stage 4
  //    input
  ...
  Kokkos::parallel_for(nverts,
                       VelocityKernel<VelocityFtor>(vertx4, vertxwork, t + dt));
  Kokkos::parallel_for(nfaces,
                       VelocityKernel<VelocityFtor>(facex4, facexwork, t + dt));
  //  Update positions
  Kokkos::parallel_for(nverts,
                       RK4Update(vertx, vertx1, vertx2, vertx3, vertx4, dt));
  Kokkos::parallel_for(nfaces,
