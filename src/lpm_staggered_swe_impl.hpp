...
void StaggeredSWE<SeedType, TopoType>::init_fields(const SurfaceType& sfc, const VorticityType& vorticity,
  ...
  Kokkos::parallel_for(mesh.n_faces_host(),
    VertexToFaceAverages<SeedType>(mesh.faces.phys_crds.view,
      double_dot_avg.view,
      grad_f_cross_u_avg.view,
      velocity_avg.view,
...
void StaggeredSWE<SeedType, TopoType>::init_direct_sums(const bool do_velocity) {
  ...
  Kokkos::parallel_for(mesh.n_vertices_host(),
    GradFCrossU<typename SeedType::geo>(grad_f_cross_u.view, mesh.vertices.lag_crds.view,
...
template <typename SeedType, typename TopoType>
void StaggeredSWE<SeedType, TopoType>::gmls_surface_laplacian(const gmls::Params& gmls_params) {
...
template <typename SeedType, typename TopoType>
void StaggeredSWE<SeedType, TopoType>::gmls_surface_laplacian(const crd_view& face_x, const gmls::Params& gmls_params) {
  crd_view face_leaf_crds("face_leaf_crds", mesh.faces.n_leaves_host());
...
template <typename SeedType, typename TopoType>
void StaggeredSWE<SeedType, TopoType>::allocate_scalar_diag(const std::string& name) {
...
template <typename SeedType, typename TopoType> template <typename SolverType>
