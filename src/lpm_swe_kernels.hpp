...
KOKKOS_INLINE_FUNCTION void kzeta_plane(
  ...
  const Real denom =
      2 * constants::PI * (square(PlaneGeometry::distance(x, y)) + square(eps));
  u[0] = -(x[1] - y[1]) / denom;
  u[1] = (x[0] - y[0]) / denom;
}
...
*/
template <typename UType, typename XType, typename YType>
KOKKOS_INLINE_FUNCTION void kzeta_sphere(UType& u, const XType& x,
...
*/
template <typename UType, typename XType, typename YType>
KOKKOS_INLINE_FUNCTION void ksigma_sphere(UType& u, const XType& x,
                                          const YType& y, const Real div_y,
                                          const Real area_y,
                                          const Real eps = 0) {
  const Real denom =
      4 * constants::PI * (1 - SphereGeometry::dot(x, y) + square(eps));
...
KOKKOS_INLINE_FUNCTION Real double_dot(const Compressed3by3& mat) {
  Real result = 0;
  for (Int i = 0; i < 3; ++i) {
    for (Int j = i; j < 3; ++i) {
...
KOKKOS_INLINE_FUNCTION Kokkos::Tuple<Real, 12> sphere_swe_velocity_sums(
    const XType& tgt_x, const YType& src_y, const Real& src_zeta,
    const Real& src_sigma, const Real& src_area, const Real& eps) {
...
*/
template <typename XType, typename YType>
KOKKOS_INLINE_FUNCTION Kokkos::Tuple<Real, 9> planar_swe_sums_rhs_pse(
  ...
  // du/dx
  result[2] = (x_mix_y * rot_str - square(x_minus_y[0]) * pot_str) / denom2 +
              pot_str / denom;
  // du/dy
  result[3] = (square(x_minus_y[1]) * rot_str - x_mix_y * pot_str) / denom2 -
              rot_str / denom;
  // dv/dx
  result[4] = (-square(x_minus_y[0]) * rot_str - x_mix_y * pot_str) / denom2 +
              rot_str / denom;
  // dv/dy
  result[5] = (-x_mix_y * rot_str - square(x_minus_y[1]) * pot_str) / denom2 +
              pot_str / denom;
...
*/
template <typename XType, typename YType>
KOKKOS_INLINE_FUNCTION Kokkos::Tuple<Real, 13> sphere_swe_sums_rhs_pse(
    const XType& tgt_x, const YType& src_y, const Real& src_zeta,
    const Real& src_sigma, const Real& src_area, const Real& src_s,
    const Real& tgt_s, const Real& eps, const Real& pse_eps) {
  using pse_type = pse::BivariateOrder8;
...
struct PlanarSwePseDirectSumReducer {
  ...
  bool collocated_src_tgt;     // [in] true if src points and target points are
                               // collocated
  ...
  PlanarSwePseDirectSumReducer(
      const crd_view tx, const scalar_view_type tsfc, const Index idx,
      const crd_view sy, const bool collocated, const scalar_view_type szeta,
      const scalar_view_type ssigma, const scalar_view_type sarea,
      const mask_view_type smask, const scalar_view_type ssfc, const Real eps,
      const Real pse_eps)
      : tgt_x(tx),
        tgt_sfc(tsfc),
        i(idx),
  ...
  void operator()(const Index& j, value_type& r) const {
    ...
    if (compute_ij) {
      ...
      const value_type local_result = planar_swe_sums_rhs_pse(
          xcrd, ycrd, src_zeta(j), src_sigma(j), src_area(j), src_sfc(j),
          tgt_sfc(i), eps, pse_eps);
      r += local_result;
    }
  }
};
...
struct SphereSweDirectSumReducer {
  ...
  SphereSweDirectSumReducer(const crd_view& tx, const Index tgt_idx,
                            const crd_view& sy, const bool colloc,
                            const scalar_view_type& sz,
                            const scalar_view_type& ss,
                            const scalar_view_type& sa, const mask_view_type m,
                            const Real eps)
      : tgt_x(tx),
        i(tgt_idx),
        src_y(sy),
  ...
  void operator()(const Index& j, value_type& r) const {
    if (!src_mask(j)) {
      if (!collocated_src_tgt or i != j) {
        const auto xcrd               = Kokkos::subview(tgt_x, i, Kokkos::ALL);
        const auto ycrd               = Kokkos::subview(src_y, j, Kokkos::ALL);
        const value_type local_result = sphere_swe_velocity_sums(
            xcrd, ycrd, src_zeta(j), src_sigma(j), src_area(j), eps);
        r += local_result;
      }
    }
  }
...
struct PlanarSWEVertexSums {
  ...
  scalar_view_type vert_s;
  ;                             /// [in] surface height at vertices
  ...
  PlanarSWEVertexSums(vec_view& vu, scalar_view_type& vdd,
                      ...
                      scalar_view_type& vls, scalar_view_type& vpsi,
                      scalar_view_type& vphi, const crd_view vx,
                      const scalar_view_type vsfc, const crd_view fy,
                      const scalar_view_type fzeta, const scalar_view_type fsig,
                      const scalar_view_type farea, const mask_view_type fmask,
                      const scalar_view_type fsfc, const Real eps,
                      const Real pse_eps, const Index nfaces,
  ...
  void operator()(const member_type& thread_team) const {
    ...
    Kokkos::parallel_reduce(
        Kokkos::TeamVectorRange(thread_team, nfaces),
        PlanarSwePseDirectSumReducer(vert_x, vert_s, i, face_y, collocated,
                                     face_zeta, face_sigma, face_area,
                                     face_mask, face_s, eps, pse_eps),
        sums);
...
struct SphereVertexSums {
  ...
  SphereVertexSums(vec_view& vu, scalar_view_type& vdd, const crd_view& vx,
                   const crd_view& fy, const scalar_view_type& fz,
  ...
  void operator()(const member_type& thread_team) const {
    ...
    Kokkos::parallel_reduce(
        ...
        SphereSweDirectSumReducer(vert_x, i, face_y, collocated, face_zeta,
                                  face_sigma, face_area, face_mask, eps),
        sums);
...
struct PlanarSWEFaceSums {
  ...
  PlanarSWEFaceSums(vec_view& fu, scalar_view_type& fdd,
                    scalar_view_type du1dx1, scalar_view_type du1dx2,
                    scalar_view_type du2dx1, scalar_view_type du2dx2,
                    scalar_view_type& fls, scalar_view_type& fpsi,
                    scalar_view_type& fphi, const crd_view fxy,
                    const scalar_view_type fzeta, const scalar_view_type fsig,
                    const scalar_view_type farea, const mask_view_type fmask,
                    const scalar_view_type fsfc, const Real eps,
                    const Real pse_eps, const Index nfaces,
...
struct SphereFaceSums {
  ...
  SphereFaceSums(vec_view& fu, scalar_view_type& fdd, const crd_view& fxy,
...
struct SWEVorticityDivergenceHeightTendencies {
  ...
  SWEVorticityDivergenceHeightTendencies(
      scalar_view_type& dzeta, scalar_view_type& dsigma, scalar_view_type& dh,
      const crd_view& x, const vec_view& u, const scalar_view_type zeta,
      const scalar_view_type sigma, const scalar_view_type h,
      const scalar_view_type ddot, const scalar_view_type laps,
  ...
  void operator()(const Index i) const {
    ...
    if constexpr (std::is_same<Geo, SphereGeometry>::value) {
      dsigma(i) = (f * zeta(i) + coriolis.grad_f_cross_u(xi, ui) - ddot(i) -
                   g * laps(i) - SphereGeometry::norm2(ui)) *
                  dt;
    } else {
      dsigma(i) = (f * zeta(i) + coriolis.grad_f_cross_u(xi, ui) - ddot(i) -
                   g * laps(i)) *
...
struct SWEVorticityDivergenceAreaTendencies {
  ...
  SWEVorticityDivergenceAreaTendencies(
      scalar_view_type& dzeta, scalar_view_type& dsigma,
      scalar_view_type& darea, const crd_view& x, const vec_view& u,
      const scalar_view_type zeta, const scalar_view_type sigma,
      const scalar_view_type area, const scalar_view_type ddot,
      const scalar_view_type laps, const coriolis_type& cor, const Real g,
      const Real dt)
      : dzeta(dzeta),
        dsigma(dsigma),
        darea(darea),
...
struct SetSurfaceFromDepth {
  ...
  SetSurfaceFromDepth(scalar_view_type& s, scalar_view_type& b,
                      const crd_view x, const scalar_view_type h,
                      const TopoType& topo)
      : s(s), b(b), x(x), h(h) {}
...
struct SetDepthAndSurfaceFromMassAndArea {
  ...
  SetDepthAndSurfaceFromMassAndArea(scalar_view_type& h, scalar_view_type& s,
                                    scalar_view_type& b, const crd_view x,
