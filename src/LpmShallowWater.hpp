...
class ShallowWater : public PolyMesh2d<SeedType> {
 public:
  ...
  ShallowWater(const Index nmaxverts, const Index nmaxedges,
               const Index nmaxfaces, const Int& nq_scaler = 0,
               const Int& nq_vector = 0);

  template <typename Geo, typename CV>
  KOKKOS_INLINE_FUNCTION
      typename std::enable_if<std::is_same<Geo, PlaneGeometry>::value,
                              Real>::type
      coriolis_f(const CV v) const {
    return f0 + beta * v[1];
  }
  ...
  inline void set_coriolis(const Real& f, const Real& b) {
    f0 = f;
    beta = b;
  }
  ...
  std::string infoString(const std::string& label = "",
                         ...
                         const bool& dump_all = false) const override;
