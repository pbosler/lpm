...
class NodeArrayD {
 public:
  ...
  ko::View<Index* [2]>
      node_pt_inds; /** node_pt_inds(i,0) = address of first point (in
                       sorted_pts) contained by node i node_pt_inds(i,1) =
                       number of points contained by node i */
  ko::View<Index*> node_parents;  /// allocated here; set by level D-1

  ko::View<Index*>
      pt_in_node;  /// pt_in_node(i) = index of the node that contains point i
  ko::View<Index*> orig_ids;  /// original (presort) locations of points

  NodeArrayD(const ko::View<Real* [3]>& p, const Int& d)
      : depth(d),
        sorted_pts("sorted_points", p.extent(0)),
        pt_in_node("pt_in_node", p.extent(0)),
        orig_ids("original_pt_locs", p.extent(0)),
        box("bbox") {
    init(p);
  }
