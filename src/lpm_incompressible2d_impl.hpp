...
void Incompressible2D<SeedType>::init_vorticity(
    const VorticityType& vorticity) {
  return init_vorticity(vorticity, 0, mesh.n_vertices_host(), 0,
                        mesh.n_faces_host());
}
...
void Incompressible2D<SeedType>::init_vorticity(const VorticityType& vorticity,
                                                ...
                                                const Index vert_end_idx,
                                                const Index face_start_idx,
                                                const Index face_end_idx) {
  // passive particles
  ...
  auto cor          = coriolis;
  Kokkos::parallel_for(
      "Incompressible2D::init_vorticity, passive",
  ...
  Kokkos::parallel_for(
      "Incompressible2D::init_vorticity, active",
...
void Incompressible2D<SeedType>::init_tracer(const TracerType& tracer,
                                             const std::string& tname) {
  static_assert(
      std::is_same<typename SeedType::geo, typename TracerType::geo>::value,
      "Geometry types must match.");

  const std::string name = (tname.empty() ? tracer.name() : tname);
  tracer_passive.emplace(
      name, ScalarField<VertexField>(name, velocity_passive.view.extent(0)));
  tracer_active.emplace(
      name, ScalarField<FaceField>(name, velocity_active.view.extent(0)));
...
template <typename SeedType>
template <typename TracerType>
void Incompressible2D<SeedType>::allocate_tracer(const TracerType& tracer,
                                                 const std::string& tname) {
  static_assert(
      std::is_same<typename SeedType::geo, typename TracerType::geo>::value,
      "Geometry types must match.");
...
void Incompressible2D<SeedType>::init_direct_sums() {
  ...
  Kokkos::parallel_for(
      "Incompressible2D::init_direct_sums, passive", passive_policy,
      Incompressible2DPassiveSums<geo>(
          velocity_passive.view, stream_fn_passive.view,
          mesh.vertices.phys_crds.view, mesh.faces.phys_crds.view,
          rel_vort_active.view, mesh.faces.area, mesh.faces.mask, eps,
          mesh.n_faces_host()));

  Kokkos::parallel_for(
      "Incompressible2D::init_direct_sums, active", active_policy,
      Incompressible2DActiveSums<geo>(
          velocity_active.view, stream_fn_active.view,
          mesh.faces.phys_crds.view, rel_vort_active.view, mesh.faces.area,
...
std::string Incompressible2D<SeedType>::info_string(const int tab_level) const {
  ...
  const std::string label =
      "Incompressible2D<" + SeedType::id_string() + "> info:\n";
...
VtkPolymeshInterface<SeedType> vtk_mesh_interface(
  ...
  for (const auto& tracer : ic2d.tracer_passive) {
    ...
    vtk.add_scalar_cell_data(ic2d.tracer_active.at(tracer.first).view,
                             tracer.first);
  }
  return vtk;
}
...
BivarRemesh<SeedType> bivar_remesh(Incompressible2D<SeedType>& new_ic2d,
  ...
  using passive_scalar_field_map =
      std::map<std::string, ScalarField<VertexField>>;
  using active_scalar_field_map = std::map<std::string, ScalarField<FaceField>>;
  using passive_vector_field_map =
      std::map<std::string, VectorField<PlaneGeometry, VertexField>>;
  using active_vector_field_map =
      std::map<std::string, VectorField<PlaneGeometry, FaceField>>;

  Kokkos::deep_copy(new_ic2d.ref_crds_passive.view,
                    new_ic2d.mesh.vertices.phys_crds.view);
  Kokkos::deep_copy(new_ic2d.ref_crds_active.view,
                    new_ic2d.mesh.faces.phys_crds.view);
  ...
  BivarRemesh<SeedType> bivar(new_ic2d.mesh, passive_scalars_new,
                              active_scalars_new, passive_vectors_new,
                              active_vectors_new, old_ic2d.mesh,
                              passive_scalars_old, active_scalars_old,
                              passive_vectors_old, active_vectors_old);
...
CompadreRemesh<SeedType> compadre_remesh(
  ...
  return CompadreRemesh<SeedType>(
      new_ic2d.mesh, passive_scalars_new, active_scalars_new,
      passive_vectors_new, active_vectors_new, old_ic2d.mesh,
      passive_scalars_old, active_scalars_old, passive_vectors_old,
      active_vectors_old, gmls_params);
}
