...
void MeshSeed<SeedType>::read_file() {
  ...
  while (std::getline(file, line)) {
    ...
    if (lineNumber > 1 && lineNumber < ncrds + 2) {
      ...
      if (crdErr) {
        oss << "MeshSeed::read_file error: cannot read coordinate from line "
            << lineNumber << " of file " << full_filename();
        LPM_REQUIRE_MSG(false, oss.str());
      }
      ...
      ++edgeCounter;
      if (edgeErr) {
        oss << "MeshSeed::read_file error: cannot read edge from line "
            << lineNumber << " of file " << full_filename();
        LPM_REQUIRE_MSG(false, oss.str());
      }
      ...
      switch (SeedType::nfaceverts) {
        ...
        case (4): {
          if (!(iss >> v0 >> v1 >> v2 >> v3)) faceErr = true;
          seed_face_verts(faceVertCounter, 0) = v0;
          seed_face_verts(faceVertCounter, 1) = v1;
          seed_face_verts(faceVertCounter, 2) = v2;
        ...
        case (5): {
          if (!(iss >> v0 >> v1 >> v2 >> v3 >> v4)) faceErr = true;
          seed_face_verts(faceVertCounter, 0) = v0;
          seed_face_verts(faceVertCounter, 1) = v1;
          seed_face_verts(faceVertCounter, 2) = v2;
          seed_face_verts(faceVertCounter, 3) = v3;
      ...
      if (faceErr) {
        oss << "MeshSeed::read_file error: cannot read face vertices from line "
            << lineNumber << " of file " << full_filename();
        LPM_REQUIRE_MSG(false, oss.str());
      }
      ...
      switch (SeedType::nfaceverts) {
        ...
        case (4): {
          if (!(iss >> e0 >> e1 >> e2 >> e3)) faceErr = true;
          seed_face_edges(faceEdgeCounter, 0) = e0;
          seed_face_edges(faceEdgeCounter, 1) = e1;
          seed_face_edges(faceEdgeCounter, 2) = e2;
        ...
        case (5): {
          if (!(iss >> e0 >> e1 >> e2 >> e3 >> e4)) faceErr = true;
          seed_face_edges(faceEdgeCounter, 0) = e0;
          seed_face_edges(faceEdgeCounter, 1) = e1;
          seed_face_edges(faceEdgeCounter, 2) = e2;
          seed_face_edges(faceEdgeCounter, 3) = e3;
      ...
      if (faceErr) {
        oss << "MeshSeed::read_file error: cannot read face edges from line "
            << lineNumber << " of file " << full_filename();
        LPM_REQUIRE_MSG(false, oss.str());
      }
      ...
      switch (SeedType::vertex_degree) {
        case (4): {
          if (!(iss >> e0 >> e1 >> e2 >> e3)) dualError = true;
        ...
        case (6): {
          if (!(iss >> e0 >> e1 >> e2 >> e3 >> e4 >> e5)) dualError = true;
          seed_vert_edges(vertCounter, 0) = e0;
          seed_vert_edges(vertCounter, 1) = e1;
          seed_vert_edges(vertCounter, 2) = e2;
          seed_vert_edges(vertCounter, 3) = e3;
      ...
      if (dualError) {
        oss << "MeshSeed::read_file error: cannot read vertex edges from line "
            << lineNumber << " of file " << full_filename();
        LPM_REQUIRE_MSG(false, oss.str());
      }
    }
...
template <typename SeedType>
void MeshSeed<SeedType>::set_max_allocations(Index& nboundary, Index& nedges,
                                             Index& nfaces,
                                             const Int lev) const {
  ...
  for (int i = 0; i <= lev; ++i) {
    ...
    nedges +=
        SeedType::n_edges_at_tree_level(SeedType::n_vertices_at_tree_level(i),
                                        SeedType::n_faces_at_tree_level(i));
  }
}
...
Real MeshSeed<SeedType>::face_area(const Int ind) const {
  ...
  ko::View<Real[SeedType::nfaceverts][SeedType::geo::ndim], Host> vertcrds(
      "vertcrds");
