...
void triangular_barycentric_coords(VT& bc, const CVT1& pt, const CVT2& pts, const Tri& tri) {
...
KOKKOS_INLINE_FUNCTION
void quad_bilinear_to_ref(Real& xi, Real& eta, const VT pt,
  const CVT& pts, const Quad& quad) {

  const Real a1 = -pts(quad(0),0) + pts(quad(1),0) - pts(quad(2),0) + pts(quad(3),0);
  const Real a2 = -pts(quad(0),1) + pts(quad(1),1) - pts(quad(2),1) + pts(quad(3),1);
  const Real b1 = -pts(quad(0),0) - pts(quad(1),0) + pts(quad(2),0) + pts(quad(3),0);
  const Real b2 = -pts(quad(0),1) - pts(quad(1),1) + pts(quad(2),1) + pts(quad(3),1);
  const Real c1 =  pts(quad(0),0) - pts(quad(1),0) - pts(quad(2),0) + pts(quad(3),0);
  const Real c2 =  pts(quad(0),1) - pts(quad(1),1) - pts(quad(2),1) + pts(quad(3),1);
  const Real d1 = 4*pt(0) - (pts(quad(0),0) + pts(quad(1),0) + pts(quad(2),0) + pts(quad(3),0));
  const Real d2 = 4*pt(1) - (pts(quad(0),1) + pts(quad(1),1) + pts(quad(2),1) + pts(quad(3),1));
...
KOKKOS_INLINE_FUNCTION
void quad_ref_to_bilinear(VT& crds, const Real& xi, const Real& eta,
  const CVT& pts, const Quad& e) {
...
KOKKOS_INLINE_FUNCTION
void quad_ref_coords(Real& a, Real& b,
  const VT1& pt, const VT2& pts, const Quad& quad) {
...
struct NativeInterpolator {
  ...
  const typename Faces<typename PolymeshType::FaceType,
                 typename PolymeshType::seed_type::geo>::vertex_view_type face_verts;
  const typename Faces<typename PolymeshType::FaceType,
                 typename PolymeshType::seed_type::geo>::face_tree_view face_kids,
  const typename Faces<typename PolymeshType::FaceType;
                 typename PolymeshType::seed_type::geo>::edge_view_type face_edges;


  NativeInterpolator(FieldType& dst,
    const typename PolymeshType::seed_type::geo::crd_view_type dst_crds,
    const FieldType& src,
    const PolymeshType& pm) :
    tgt_view(dst.view),
    tgt_crds(dst_crds),
  ...
  operator () (const Index tgt_idx) const {
    ...
    const Index f_idx = locate_face_containing_pt<decltype(m_crd), TriFace>(
      m_crd, edge_lefts, edge_rights, edge_kids, face_crds, face_kids, face_edges);

    const auto m_tri = Kokkos::subview(face_verts, f_idx, Kokkos::ALL);

    Real bc[3];
    triangular_barycentric_coords<typename PolymeshType::Geo, Real*,
      decltype(m_crd), decltype(vert_crds), decltype(m_tri)>(bc,
      m_crd, vert_crds, m_tri);
  ...
  typename std::enable_if<
    std::is_same<typename PolymeshType::FaceType, QuadFace>::value, void>::type
  KOKKOS_INLINE_FUNCTION
