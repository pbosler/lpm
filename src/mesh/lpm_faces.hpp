...
class NcWriter;  // fwd decl
class PolymeshReader;
#endif

/** @brief Faces define panels.  Connected to Coords and Edges.

Most faces have only vertices (define by two sets of Coords, one physical, one
Lagrangian) and single particles initialized at their barycrd_inds (also 2 sets
of Coords, but separate from the vertcies). High order faces have multiple
particles in their interior.
...
class Faces {
 public:
  ...
  template <typename Geom, typename FaceType>
  friend struct FaceDivider;
  ...
  vertex_view_type
      verts;  ///< indices to Coords on face edges, ccw order per face
  ...
  */
  ...
  @hostfn If the device has updated face areas, this function will not see it
  unless update_host() is called first.
  */
  ...
  */
  ...
  /** @brief Get a particular vertex from a host
    ...
    @param relInd relative index of vertex (relative to face(ind) in Faces
    object); see MeshSeed
  */
  Index vert_host(const Index ind, const Int relInd) const {
    return _hostverts(ind, relInd);
  }
  ...
  Index kid_host(const Index idx, const Int kid) const {
    return _hostkids(idx, kid);
  }
  ...
  /** @brief Get a particular edge from a host
   ...
   @param ind index of face
   @param relInd relative index of edge (relative to face(ind) in Faces object);
   see MeshSeed
 */
  Index edge_host(const Index ind, const Int relInd) const {
    return _hostedges(ind, relInd);
  }

  /** @brief Get the index of the center particle (from Coords) associated with
  a face
  ...
  */
  inline Index crd_idx_host(const Index ind) const {
    return _host_crd_inds(ind);
  }
  ...
  */
  inline bool edge_is_positive(const Index faceInd, const Int relEdgeInd,
  ...
  */
  inline void set_area_host(const Index ind, const Real ar) {
    _hostarea(ind) = ar;
  }
  ...
  */
  ...
  @hostfn If the device updates face areas, this function will  not see it
  unless update_host() is called first.
  */
...
template <typename Geo>
struct FaceDivider<Geo, TriFace> {
  static void divide(const Index faceInd, Vertices<Coords<Geo>>& verts,
                     Edges& edges, Faces<TriFace, Geo>& faces);
};

template <typename Geo>
struct FaceDivider<Geo, QuadFace> {
  static void divide(const Index faceInd, Vertices<Coords<Geo>>& verts,
