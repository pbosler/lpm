...
void Edges::insert_host(const Index o, const Index d, const Index l,
                        const Index r, const Index prt) {
...
void Edges::divide(const Index edge_idx, Vertices<CoordsType>& verts) {
  ...
  LPM_REQUIRE_MSG(_nh() + 2 <= _nmax,
                  "Edges::divide error, not enough memory.");
  ...
  Geo::midpoint(lag_midpt, ko::subview(lag_endpts, 0, ko::ALL),
                ko::subview(lag_endpts, 1, ko::ALL));
  // insert new midpoint to vertices
  verts.insert_host(midpt, lag_midpt);
  // insert new child edges
  this->insert_host(_ho(edge_idx), verts_insert_idx, _hl(edge_idx),
                    _hr(edge_idx), edge_idx);
  this->insert_host(verts_insert_idx, _hd(edge_idx), _hl(edge_idx),
                    _hr(edge_idx), edge_idx);
...
//   Coords<CircularPlaneGeometry>& crds, Coords<CircularPlaneGeometry>&
//   lagcrds) {
//
//   assert(ind < _nh());
//
//   LPM_REQUIRE_MSG(_nh()+2 <= _nmax, "Edges::divide error: not enough
//   memory."); LPM_REQUIRE_MSG(!has_kids_host(ind), "Edges::divide error:
//   called on previously divided edge.");
//
...
//   }
//   const Real lr0 =
//   CircularPlaneGeometry::mag(ko::subview(lagendpts,0,ko::ALL())); const Real
//   lr1 = CircularPlaneGeometry::mag(ko::subview(lagendpts,1,ko::ALL())); if
//   (FloatingPoint<Real>::equiv(lr0, lr1, 10*constants::ZERO_TOL)) {
//     CircularPlaneGeometry::radial_midpoint(midpt, ko::subview(endpts, 0,
//     ko::ALL()),
//       ko::subview(endpts,1,ko::ALL()));
//     CircularPlaneGeometry::radial_midpoint(lagmidpt, ko::subview(lagendpts,
//     0, ko::ALL()),
...
//       ko::subview(endpts,1,ko::ALL()));
//     CircularPlaneGeometry::midpoint(lagmidpt, ko::subview(lagendpts, 0,
//     ko::ALL()),
//       ko::subview(lagendpts,1,ko::ALL()));
//   }
...
std::string Edges::info_string(const std::string& label, const short& tab_level,
                               const bool& dump_all) const {
  std::ostringstream oss;
  const auto indent = indent_string(tab_level);

  oss << indent << "Edges " << label << " info: nh = (" << _nh()
...
/// ETI
template void Edges::divide<Coords<PlaneGeometry>>(
    const Index ind, Vertices<Coords<PlaneGeometry>>& verts);

template void Edges::divide<Coords<SphereGeometry>>(
    const Index ind, Vertices<Coords<SphereGeometry>>& verts);
