...
/*
  ...
  Each type of seed below is a initializer for the tree-based meshes employed by
  LPM.
  ...
  Required static methods:
    ...
    id_string : return the name of the seed (used for console output, debugging,
  etc.) n_vertices_at_tree_level(const Int lev) : return the number of vertices
  in a mesh at tree depth = lev n_faces_at_tree_level(const Int lev) : return
  the number of leaf faces in a mesh at tree depth = lev
    n_edges_at_tree_level(const Int nv, const Int nf): return the number of leaf
  edges in a mesh with nv vertices and nf faces
*/
...
struct TriHexSeed {
  ...
  static std::string id_string() { return "tri_hex"; }
...
struct CubedSphereSeed {
  ...
  static std::string id_string() { return "cubed_sphere"; }
...
struct IcosTriSphereSeed {
  ...
  static std::string id_string() { return "icostri_sphere"; }
  static std::string face_str() { return "tri"; }
...
/** @brief A MeshSeed initializes a particle/panel mesh, and the Edges tree and
  Faces tree.
...
struct MeshSeed {
  ...
  ko::View<Index[SeedType::nverts][SeedType::vertex_degree], Host>
      seed_vert_edges;

  /// constructor.  Automatically reads data file.
  MeshSeed()
      : seed_crds("seed coords"),
        seed_edges("seed edges"),
        seed_face_verts("seed face vertices"),
        seed_face_edges("seed face edges"),
        seed_vert_edges("seed vertex edges") {
    read_file();
  }
  ...
  /** Return the required memory allocations for a mesh tree of depth = lev
    nboundary = n vertices in a typical mesh (only different for high-order
    meshes, which are not implemented yet) nedges = n edges required by the mesh
    tree nfaces = n faces required by the mesh tree
  */
  void set_max_allocations(Index& nboundary, Index& nedges, Index& nfaces,
                           const Int lev) const;
