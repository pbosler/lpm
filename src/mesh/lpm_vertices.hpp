  ...
  public:
    ...
    */
        ...
        n("n")
    {
      ...
      if (verts_are_dual) {
        edges = ko::View<Index**>("edges_at_vertex", nmax, constants::MAX_VERTEX_DEGREE);
    ...
    */
    ...
    /** @brief  Insert a new vertex
      ...
      @param [in] physcrd physical coordinates of a single point (the vertex to add)
    ...
    */
    ...
    /** @brief Array to hold indices of edges incident to each vertex. Only used for
      dual meshes.  If mesh is not dual, this view is not allocated.
    */
    ko::View<Index**> edges;

    /** @brief Smart pointer to a Coords object containing the physical coordinates
      of each vertex.
    */
    std::shared_ptr<CoordsType> phys_crds;

    /** @brief Smart pointer to a Coords object containing the Lagrangian coordinates
      of each vertex.
    */
    std::shared_ptr<CoordsType> lag_crds;

    /** @brief Create and return a string containing info about this Vertices instance.
