...
struct CompadreParams {
  ...
  CompadreParams()
      : gmls_eps_mult(2.0),
        ...
        gmls_weight_pow(2),
        gmls_manifold_weight_pow(2),
        ambient_dim(3),
        topo_dim(2),
        min_neighbors(Compadre::GMLS::getNP(gmls_order, topo_dim)) {}

  CompadreParams(const Int ord)
      : gmls_eps_mult((ord > 4 ? 3.0 : 2.0)),
        gmls_order(ord),
        gmls_manifold_order(ord),
        gmls_weight_pow(2),
        gmls_manifold_weight_pow(2),
        ambient_dim(3),
        topo_dim(2),
...
struct CompadreNeighborhoods {
  ...
  void compute_bds() {
    ...
    ko::parallel_reduce(neighborhood_radii.extent(0),
                        RadiusReducer(neighborhood_radii),
                        ko::MinMax<Real>(rr));
    ko::parallel_reduce(neighbor_lists.extent(0), NNReducer(neighbor_lists),
                        ko::MinMax<Int>(mm));
