...
struct PlanarGaussianMountain {
  ...
  template <typename CV>
  KOKKOS_INLINE_FUNCTION Real laplacian(const CV xy) const {
    return 4 * b * mtn_height * (b * PlaneGeometry::norm2(xy) - 1) *
...
struct PlanarGaussianSurfacePerturbation {
  ...
  static constexpr Real ptb_x0 =
      -1.125;                        // Center of perturbation, x-coordinate
  static constexpr Real ptb_y0 = 0;  // Center of perturbation, y-coordinate

  template <typename CV>
  KOKKOS_INLINE_FUNCTION Real operator()(const CV xy) const {
    return H0 + ptb_height * exp(-(ptb_bx * square(xy(0) - ptb_x0) +
                                   ptb_by * square(xy(1) - ptb_y0)));
  }
...
struct ZeroFunctor {
  template <typename CV>
  KOKKOS_INLINE_FUNCTION Real operator()(const CV x) const {
    return 0;
  }

  template <typename CV>
  KOKKOS_INLINE_FUNCTION Real laplacian(const CV x) const {
    return 0;
  }
...
struct SphereTestCase5Bottom {
  ...
  KOKKOS_INLINE_FUNCTION Real operator()(const CV xyz) const {
    ...
    const Real ll_dist_sq =
        square(lambda - lambda_ctr) + square(theta - theta_ctr);
