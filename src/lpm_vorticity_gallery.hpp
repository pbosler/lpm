...
struct SolidBodyRotation {
  ...
  template <typename PtView>
  KOKKOS_INLINE_FUNCTION Real operator()(const PtView& pt) const {
    return 2 * OMEGA * pt[2];
  }
...
struct GaussianVortexSphere {
  ...
  Real operator()(const Real& x, const Real& y, const Real& z) const {
    ...
    const Real zeta =
        vortex_strength * exp(-square(shape_parameter) * distsq) - gauss_const;
    return zeta;
  }

  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real operator()(const PtType& xyz) const {
    const Real distsq =
        1.0 - xyz[0] * xyz_ctr[0] - xyz[1] * xyz_ctr[1] - xyz[2] * xyz_ctr[2];
    const Real zeta =
        vortex_strength * exp(-square(shape_parameter) * distsq) - gauss_const;
    return zeta;
  }
...
struct RossbyHaurwitz54 {
  ...
  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real operator()(const PtType& xyz) const {
    return 2 * u0 * xyz[2] + 30 * rh54_amplitude *
                                 cos(4 * SphereGeometry::longitude(xyz)) *
                                 legendreP54(xyz[2]);
  }

  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real laplacian(const PtType& xyz) const {
    return -4 * u0 * xyz[2] - 900 * rh54_amplitude *
                                  cos(4 * SphereGeometry::longitude(xyz)) *
                                  legendreP54(xyz[2]);
  }
};
...
struct RossbyHaurwitzR {
  ...
  Real operator()(const Real& x, const Real& y, const Real& z) const {
    const Real lon = atan4(y, x);
    return 2 * u0 * z +
           (R + 1) * (R + 2) * rh54_amplitude * cos(R * lon) * legendrePR(z);
  }
};
//

// #ifdef LPM_USE_BOOST
// inline Real lamb_dipole_vorticity(const Real x, const Real y, const Real
// xctr,
/** Returns the vorticity of a compactly supported Lamb dipole.
  ...
  @param [in] dipole_radius radius of support (vorticity is zero outside of this
  radius)
...
Real lamb_dipole_vorticity(const Real x, const Real y, const Real xctr,
  ...
  if ((r < dipole_radius) and !FloatingPoint<Real>::zero(r)) {
    ...
    result = -2 * dipole_strength * k * bessel_j1(k * r) * sintheta / denom;
  }
  return result;
}
...
struct CollidingDipolePairPlane {
  ...
  KOKKOS_INLINE_FUNCTION
  CollidingDipolePairPlane(const Real strA, const Real rA,
                           const Kokkos::Tuple<Real, 2> ctrA, const Real strB,
                           const Real rB, const Kokkos::Tuple<Real, 2> ctrB)
  ...
  KOKKOS_INLINE_FUNCTION Real operator()(const CV& xy) const {
    return lamb_dipole_vorticity(xy[0], xy[1], xyz_ctrA[0], xyz_ctrA[1],
                                 dipole_radiusA, dipole_strengthA) +
           lamb_dipole_vorticity(xy[0], xy[1], xyz_ctrB[0], xyz_ctrB[1],
                                 dipole_radiusB, dipole_strengthB);
  }
...
struct SphereTestCase2Vorticity {
  ...
  template <typename CV>
  KOKKOS_INLINE_FUNCTION Real operator()(const CV& xyz) const {
    return 2 * u0 * xyz[2];
  }
