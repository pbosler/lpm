...
ShallowWater<SeedType>::ShallowWater(const Index nmaxverts, const Index nmaxedges,
  const Index nmaxfaces, const Int& nq_scaler, const Int& nq_vector) :
  ...
  scalar_tracer_verts(nq_scaler), scalar_tracer_faces(nq_scaler),
  vector_tracer_verts(nq_vector), vector_tracer_faces(nq_vector),
  ...
  f0(0), beta(0), Omega(0) {
    ...
    for (Int i=0; i<nq_scaler; ++i) {
      ...
      host_scalar_tracer_verts.push_back(ko::create_mirror_view(scalar_tracer_verts[i]));

      scalar_tracer_faces[i] = scalar_view_type(ss.str(), nmaxfaces);
      host_scalar_tracer_faces.push_back(ko::create_mirror_view(scalar_tracer_faces[i]));
      ss.str("");
    }
    ...
    for (Int i=0; i<nq_vector; ++i) {
      ...
      host_vector_tracer_verts.push_back(ko::create_mirror_view(vector_tracer_verts[i]));

      vector_tracer_faces[i] = vector_field(ss.str(), nmaxfaces);
      host_vector_tracer_faces.push_back(ko::create_mirror_view(vector_tracer_faces[i]));
      ss.str("");
    }
}
...
void ShallowWater<SeedType>::set_bottom_topography() {
  ...
  ko::parallel_for(this->nvertsHost(), KOKKOS_LAMBDA (const Index& i) {
    const auto mycrd = ko::subview(vccopy, i, ko::ALL);
    tvcopy(i) = ProblemType::bottom_height(mycrd);
  });

  ko::parallel_for(this->nfacesHost(), KOKKOS_LAMBDA (const Index& i) {
...
template <typename SeedType>
void ShallowWater<SeedType>::addFieldsToVtk(Polymesh2dVtkInterface<SeedType>& vtk) const {
