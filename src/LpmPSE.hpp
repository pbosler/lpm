...
static Real pse_eps(const Real& dx,
                    const Real& p = 17.0 / 20.0 /* default value p = 0.85 */) {
  assert(p < 1);
  return std::pow(dx, p);
}

template <typename Geo, typename CV>
KOKKOS_INLINE_FUNCTION static Real pse_kernel_input(const CV& x0, const CV& x1,
                                                    const Real& eps) {
...
struct PlanePSEDelta8Reduce {
  ...
  KOKKOS_INLINE_FUNCTION
  PlanePSEDelta8Reduce(const Index& ind, const crd_view& tgts,
                       const crd_view& srcs, const scalar_view_type& srcvals,
                       const scalar_view_type& a, const mask_view_type sm,
                       const Real& ep)
      : i(ind),
        tgt_ind(i),
        tgtx(tgts),
...
struct PlanePSELaplacian8Reduce {
  ...
  KOKKOS_INLINE_FUNCTION
  PlanePSELaplacian8Reduce(const Index& i, const crd_view& tx,
                           const scalar_view_type& tf, const crd_view& sx,
                           const scalar_view_type& sf,
                           const scalar_view_type& sa, const Real& pe)
      : tgt_ind(i),
        tgtx(tx),
        tgtf(tf),
        srcx(sx),
...
struct PlanePSELaplacian {
  ...
  PlanePSELaplacian(scalar_view_type& lap_out, const crd_view& tx,
                    const scalar_view_type& tf, const crd_view& sx,
                    const scalar_view_type& sf, const scalar_view_type& sa,
                    const Real& pe, const Index& ns)
      : laplacian(lap_out),
        tgtx(tx),
        tgtf(tf),
        srcx(sx),
...
struct PlanePSEScalarInterp {
  ...
  PlanePSEScalarInterp(scalar_view_type& f, const crd_view& t,
                       const crd_view& s, const scalar_view_type& fs,
                       const scalar_view_type& sa, const mask_view_type& sm,
                       const Real& ep, const Index& n)
      : finterp(f),
        tgtx(t),
        srcx(s),
        srcdata(fs),
