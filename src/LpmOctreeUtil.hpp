...
KOKKOS_INLINE_FUNCTION T pintpow2(IntT2 k) {
  assert(k >= 0);
  T result = 1;
  while (k > 0) {
    result *= 2;
...
template <typename T = key_type, typename IntT2 = int>
KOKKOS_INLINE_FUNCTION T pintpow8(IntT2 k) {
  assert(k >= 0);
  T result = 1;
  while (k > 0) {
    result *= 8;
...
/**
    ...
    where bits xi, yi, zi, correspond to left (0) and right(1) of the centroid
   of octree node i in the x,y,z direction
*/
template <typename CPtType>
KOKKOS_INLINE_FUNCTION key_type compute_key_for_point(const CPtType& pos,
                                                      const int& depth,
                                                      BBox bb) {
...
key_type parent_key(const key_type& k, const int& lev,
...
KOKKOS_INLINE_FUNCTION
key_type local_key(const key_type& k, const int& lev,
                   const Int& max_depth = MAX_OCTREE_DEPTH) {
  assert(max_depth > 0 && max_depth <= MAX_OCTREE_DEPTH);
  assert(lev > 0 && lev <= max_depth);
  const key_type nbits = 3 * max_depth;
...
KOKKOS_INLINE_FUNCTION
BBox box_from_key(const key_type& k, const BBox& rbox, const Int& lev,
                  const Int& max_depth) {
  assert(max_depth > 0 && max_depth <= MAX_OCTREE_DEPTH);
  assert(lev > 0 && lev <= max_depth);
  Real cx, cy, cz;
  ...
  return BBox(cx - half_len, cx + half_len, cy - half_len, cy + half_len,
              cz - half_len, cz + half_len);
}

KOKKOS_INLINE_FUNCTION
key_type node_key(const key_type& pk, const key_type& lk, const int& lev,
                  const int& max_depth) {
...
KOKKOS_INLINE_FUNCTION
id_type decode_id(const code_type& code) { return id_type(code); }

KOKKOS_INLINE_FUNCTION
key_type decode_key(const code_type& code) { return key_type((code >> 32)); }

template <typename CVT>
KOKKOS_INLINE_FUNCTION Index binarySearchCodes(const key_type& key,
                                               const CVT& sorted_codes,
...
template <typename CVT>
KOKKOS_INLINE_FUNCTION Index binarySearchKeys(const key_type& key,
                                              const CVT& sorted_keys,
                                              const bool& get_first) {
  Index low = 0;
