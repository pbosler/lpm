...
KOKKOS_INLINE_FUNCTION void velocityKernel(
    ko::Tuple<Real, 2>& u, const VecType& tgt_x, const VecType& src_x,
...
template <typename VecType>
KOKKOS_INLINE_FUNCTION void planeSweRhsPse(
    ko::Tuple<Real, 7>& res, const VecType& tgt_x, const Real& tgt_s,
  ...
  if (sqdist > 1E-12) {
    ...
    const Real pse_scaled_r =
        pse_kernel_input<PlaneGeometry, VecType>(tgt_x, src_x, pse_eps);
    const Real lap_ker = bivariateLaplacianOrder8(pse_scaled_r);

    // u = velocity, x component
    res[0] = -(tgt_x[1] - src_x[1]) * rot_strength +
             (tgt_x[0] - src_x[0]) * pot_strength;
    // v = velocity, y component
    res[1] = (tgt_x[0] - src_x[0]) * rot_strength +
             (tgt_x[1] - src_x[1]) * pot_strength;
    // du/dx
    res[2] = pot_strength - (tgt_x[0] - src_x[0]) *
                                ((tgt_x[0] - src_x[0]) * src_div -
                                 (tgt_x[1] - src_x[1]) * src_vort) *
                                src_area / denom2;
    // du/dy
    res[3] = -rot_strength - (tgt_x[1] - src_x[1]) *
                                 ((tgt_x[0] - src_x[0]) * src_div -
                                  (tgt_x[1] - src_x[1]) * src_vort) *
                                 src_area / denom2;
    // dv/dx
    res[4] = rot_strength - (tgt_x[0] - src_x[0]) *
                                ((tgt_x[1] - src_x[1]) * src_div +
                                 (tgt_x[0] - src_x[0]) * src_vort) *
                                src_area / denom2;
    // dv/dy
    res[5] = pot_strength - (tgt_x[1] - src_x[1]) *
                                ((tgt_x[1] - src_x[1]) * src_div +
                                 (tgt_x[0] - src_x[0]) * src_vort) *
                                src_area / denom2;
    // laplacian(s)
...
struct PlanarSWEDirectSum {
  ...
  PlanarSWEDirectSum(const Index& tind, const crd_view& tx,
                     const scalar_view_type& tgtsfc, const crd_view& sx,
                     const scalar_view_type& z, const scalar_view_type& sdiv,
                     const scalar_view_type& a, const scalar_view_type& ssfc,
                     const Real& eps)
      : i(tind),
        tgtx(tx),
        tgt_sfc(tgtsfc),
  ...
  void operator()(const Index& j, value_type& r) const {
    if (!collocated_src_tgt || i != j) {
      ...
      planeSweRhsPse(lsum, mtgt, tgt_sfc(i), msrc, src_zeta(j), src_sigma(j),
                     src_area(j), src_sfc(j), pse_eps);
      r += lsum;
    }
  }
};
...
struct PlanarSWEVertexSums {
  ...
  PlanarSWEVertexSums(vec_view& vvel, scalar_view_type& vdd,
                      scalar_view_type& vlap, const crd_view& vx,
                      const scalar_view_type& vsfc, const crd_view& fx,
                      const scalar_view_type& fz, const scalar_view_type& fdiv,
                      const scalar_view_type& fa, const scalar_view_type& fsfc,
                      const Real& ep)
      : vertvel(vvel),
        vertddot(vdd),
        vertlaps(vlap),
  ...
  void operator()(const member_type& mbr) const {
    ...
    ko::parallel_reduce(ko::TeamThreadRange(mbr, nf),
                        PlanarSWEDirectSum(i, vertx, vertsfc, facex, facevort,
                                           facediv, facearea, facesfc, eps),
                        red);
    vertvel(i, 0) = red[0];
    vertvel(i, 1) = red[1];
...
struct PlanarSWEVertexRHS {
  ...
  PlanarSWEVertexRHS(crd_view& dx_, scalar_view_type& dz,
                     scalar_view_type& dsig, scalar_view_type& ddepth,
                     const crd_view& vx, const vec_view& uv,
                     const scalar_view_type& vvort,
                     const scalar_view_type& vdiv, const scalar_view_type& vdd,
                     const scalar_view_type& vls, const scalar_view_type& h,
                     const Real& ff, const Real& bb, const Real& gg,
                     const Real& dt_)
      : dx(dx_),
        dzeta(dz),
        dsigma(dsig),
  ...
  void operator()(const Index& i) const {
    ...
    dsigma(i) =
        (hasmass ? dt * (-f * vertvort(i) - vertddot(i) - g * vertlaps(i)) : 0);
...
struct PlanarSWESetVertexSfc {
  ...
  PlanarSWESetVertexSfc(scalar_view_type& s, scalar_view_type& sb,
                        const scalar_view_type& h, const crd_view& vx)
      : sfc(s), topo(sb), depth(h), vertx(vx) {}
...
struct PlanarSWESetFaceSfc {
  ...
  PlanarSWESetFaceSfc(scalar_view_type& s, scalar_view_type& h,
                      scalar_view_type& sb, const scalar_view_type& mm,
...
struct PlanarSWEFaceSums {
  ...
  PlanarSWEFaceSums(vec_view& fv, scalar_view_type& fdd, scalar_view_type flap,
                    const crd_view& fx, const scalar_view_type& fz,
                    const scalar_view_type& fdiv, const scalar_view_type& fa,
                    const scalar_view_type& fsfc, const Real& ep)
      : facevel(fv),
        faceddot(fdd),
        facelaps(flap),
        facex(fx),
  ...
  void operator()(const member_type& mbr) const {
    ...
    ko::Tuple<Real, 7> red;
    ko::parallel_reduce(ko::TeamThreadRange(mbr, nf),
                        PlanarSWEDirectSum(i, facex, facesfc, facex, facevort,
                                           facediv, facearea, facesfc, eps),
                        red);
    facevel(i, 0) = red[0];
    facevel(i, 1) = red[1];
...
struct PlanarSWEFaceRHS {
  ...
  PlanarSWEFaceRHS(crd_view& dx_, scalar_view_type& dz, scalar_view_type& dsig,
                   scalar_view_type& da, const crd_view& fx, const vec_view& uv,
                   const scalar_view_type& fzeta, const scalar_view_type& fdiv,
                   const scalar_view_type& fdd, const scalar_view_type& flaps,
                   const scalar_view_type& fa, const mask_view_type& fm,
                   const Real& ff, const Real& bb, const Real& gg,
                   const Real& dt_)
      : dx(dx_),
