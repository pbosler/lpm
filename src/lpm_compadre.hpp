...
Compadre::GMLS plane_scalar_gmls(const SrcCrdViewType& src_crds,
  const TgtCrdViewType& tgt_crds,
  const Neighborhoods& nn,
  const Params& params,
  const std::vector<Compadre::TargetOperation>& ops) {

  constexpr auto reconstruction_space = Compadre::ReconstructionSpace::ScalarTaylorPolynomial;
  ...
  Compadre::GMLS result(reconstruction_space,
    sampling_functional,
    data_functional,
...
template <typename SrcCrdViewType, typename TgtCrdViewType>
Compadre::GMLS sphere_scalar_gmls(const SrcCrdViewType src_crds,
  const TgtCrdViewType tgt_crds,
  const Neighborhoods& nn,
  const Params& params,
  const std::vector<Compadre::TargetOperation>& ops) {
