...
template <typename V1, typename V2, typename V3, int Rank> struct
ComputeErrorFtor {
  V1 err;
  V2 appx;
  V3 exact;
...
template <typename V1, typename V2, typename V3> struct
ComputeErrorFtor<V1, V2, V3, 1> {
  V1 err;
  V2 appx;
  V3 exact;
  Int ndim;
...
template <typename V1, typename V2, int Rank> struct
ReduceErrorFtor {
  ...
  void operator() (const Index i, ENormScalar& ll) const {
    ...
    const Real err_mag = (err.extent(1) == 2 ? PlaneGeometry::mag(my_err) : SphereGeometry::mag(my_err));
    const Real exact_mag = (err.extent(1) == 2 ? PlaneGeometry::mag(my_exact) : SphereGeometry::mag(my_exact));
...
template <typename V1, typename V2> struct
ReduceErrorFtor<V1, V2, 1> {
  V1 err;
  V2 exact;
  scalar_view_type weight;
  Int ndim;
  ...
  void operator() (const Index i, ENormScalar& ll) const {
    ...
    ll.linfdenom = (abs(exact(i)) > ll.linfdenom ? abs(exact(i)) : ll.linfdenom);
  }
};
...
void compute_error(const V1 err, const V2 appx, const V3 exact) {
  ...
  LPM_REQUIRE(err.extent(0) == appx.extent(0) and appx.extent(0) == exact.extent(0));
  LPM_REQUIRE(err.extent(1) == appx.extent(1) and appx.extent(1) == exact.extent(1));

  Kokkos::parallel_for(err.extent(0),
    ComputeErrorFtor<V1, V2, V3, V1::Rank>(err, appx, exact));
}
...
ENormScalar reduce_error(const V1 err, const V2 exact, const scalar_view_type wt) {
  ...
  LPM_REQUIRE(err.extent(0) == exact.extent(0) and exact.extent(0) == wt.extent(0));
  LPM_REQUIRE(err.extent(1) == exact.extent(1));
  Kokkos::parallel_reduce("reduce error (vector)", err.extent(0),
    ReduceErrorFtor<V1, V2, V1::Rank>(err, exact, wt), ErrReducer<Host>(rval));
  return rval;
}

template <typename V1, typename V2, typename V3>
ErrNorms::ErrNorms(const V1 err, const V2 appx, const V3 exact, const scalar_view_type wt) {
