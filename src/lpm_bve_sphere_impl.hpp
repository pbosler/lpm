...
VtkPolymeshInterface<SeedType> vtk_interface(const std::shared_ptr<BVESphere<SeedType>> bve) {
    ...
    for (Short i=0; i<bve->tracer_verts.size(); ++i) {
      vtk.add_scalar_point_data(bve->tracer_verts[i].view, bve->tracer_verts[i].view.label());
      vtk.add_scalar_cell_data(bve->tracer_faces[i].view, bve->tracer_faces[i].view.label());
    }
    return vtk;
}
#endif
...
void BVESphere<SeedType>::init_velocity() {
  ...
  Kokkos::parallel_for("BVESphere::init_velocity_vertices", vertex_policy,
    BVEVertexVelocity(velocity_verts.view, this->vertices.phys_crds->crds,
      this->faces.phys_crds->crds, rel_vort_faces.view, this->faces.area, this->faces.mask,
      this->n_faces_host()));

  Kokkos::parallel_for("BVESphere::init_velocity_faces", face_policy,
    BVEFaceVelocity(velocity_faces.view, this->faces.phys_crds->crds, rel_vort_faces.view,
...
void BVESphere<SeedType>::init_stream_fn() {
  ...
  Kokkos::parallel_for("BVESphere::init_stream_fn_vertices", vertex_policy,
    BVEVertexStreamFn(stream_fn_verts.view, this->vertices.phys_crds->crds,
      this->faces.phys_crds->crds, rel_vort_faces.view, this->faces.area,
  ...
  Kokkos::parallel_for("BVESphere::init_stream_fn_faces", face_policy,
    BVEFaceStreamFn(stream_fn_faces.view, this->faces.phys_crds->crds,
      rel_vort_faces.view, this->faces.area, this->faces.mask, this->n_faces_host()));
