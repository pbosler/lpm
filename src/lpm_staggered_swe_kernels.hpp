...
struct StaggeredSWEKernels {
  ...
  KOKKOS_INLINE_FUNCTION static Real greens(const XType& x, const YType& y,
                                            const Real& eps) {
  ...
  template <typename XType, typename YType>
  KOKKOS_INLINE_FUNCTION static vec_type kzeta(const XType& x, const YType& y,
                                               const Real& eps) {
    const Real denom =
  ...
  template <typename XType, typename YType>
  KOKKOS_INLINE_FUNCTION static vec_type ksigma(const XType& x, const YType& y,
                                                const Real& eps) {
    const Real denom =
        -4 * constants::PI * (1 - SphereGeometry::dot(x, y) + square(eps));
  ...
  template <typename XType, typename YType>
  KOKKOS_INLINE_FUNCTION static mat_type grad_kzeta(const XType& x,
                                                    const YType& y,
                                                    const Real& eps) {
    const mat_type proj_mat = spherical_tangent_projection_matrix(x);
    const Real denom =
        4 * constants::PI * (1 - SphereGeometry::dot(x, y) + square(eps));
    const mat_type permuted_y = {0,    y[2], -y[1], -y[2], 0,
                                 y[0], y[1], -y[0], 0};
    mat_type term1;
  ...
  template <typename XType, typename YType>
  KOKKOS_INLINE_FUNCTION static mat_type grad_ksigma(const XType& x,
                                                     const YType& y,
                                                     const Real& eps) {
    const mat_type proj_mat = spherical_tangent_projection_matrix(x);
    const Real denom =
