...

{
  ...
  gathered_laplacian = scalar_view_type("surface_laplacian", gathered_mesh->n());
  ...
  scatter_mesh = std::make_unique<ScatterMeshData<SeedType>>(*gathered_mesh, swe.mesh);
  neighbors = gmls::Neighborhoods(gathered_mesh->h_phys_crds,
    gathered_mesh->h_phys_crds, params);
}

template <typename SeedType>
void SWEGMLSLaplacian<SeedType>::update_src_data(const crd_view xp, const crd_view xa,
