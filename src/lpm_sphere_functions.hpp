...
KOKKOS_INLINE_FUNCTION void greens_fn(Real& psi, const VecType& tgt_x,
                                      const VecType& src_x,
                                      const Real& src_vort,
                                      const Real src_area) {
...
*/
template <typename VecType>
KOKKOS_INLINE_FUNCTION void biot_savart(ko::Tuple<Real, 3>& u,
                                        const VecType& tgt_x,
                                        const VecType& src_x,
                                        const Real& src_vort,
                                        const Real& src_area) {
...
struct SphereTangentFunctor {
  ...
  SphereTangentFunctor(scalar_view_type& dp,
                       const SphereGeometry::crd_view_type& pos,
                       const SphereGeometry::vec_view_type& vel)
      : udotx(dp), x(pos), u(vel) {}
