...
void NodeArrayD::init(const ko::View<Real* [3]>& presorted_pts) {
  ...
  */
  ...
  /**
      Input: points
      Process: compute 32-bit node key of each point, concatenate 32-bit point
     id into 64-bit code Kernel : EncodeFunctor Loop over: points Output:
     encoded point key/id pairs Sort code array
  */
  ...
  */
  ko::sort(pt_codes);
  ko::parallel_for(
      npts, PermuteFunctor(sorted_pts, orig_ids, presorted_pts, pt_codes));
  ...
  /**
      ...
      Process:
          Kernel: MarkDuplicates
              ...
              a. Flag each unique node using encoded keys (flag = 1 for unique
     node; 0 otherwise) b. Inclusive scan to count the number of unique nodes c.
     After scan, flags(npts-1) = count of nodes that have points

          Kernel: UniqueNodeFunctor
              Loop over: point codes
              For each node, find index of first contained point, number of
     points contained Output: unique node keys point start, count
  */
  ...
  /**
      ...
      Equivalently: Reserve space for each unique node to have a full set of 8
     siblings. Process: Kernel: NodeSiblingCounter Loop over: unique keys a.
     Flag each node with a new parent (8 = new parent, 0 otherwise) b. Inclusive
     scan to count the number of nodes that need to be allocated
  */
  ...
  ko::parallel_scan(
      ko::RangePolicy<NodeSiblingCounter::ScanTag>(0, unode_count),
      NodeSiblingCounter(nsiblings, ukeys, depth, depth));
  ...
  ko::parallel_for(
      unode_count,
      NodeArrayDFunctor(node_keys, node_pt_inds, node_parents, pt_in_node,
                        nsiblings, ukeys, uinds, depth));

#ifdef LPM_ENABLE_DEBUG
  std::cout << "NodeArrayD::init: nnodes = " << nnodes << " out of "
            << pintpow8(depth) << " possible.\n";
  Index npts_check = 0;
  const auto loc_pt_inds = node_pt_inds;
  ko::parallel_reduce(
      nnodes,
      KOKKOS_LAMBDA(const Index& i, Index& ct) { ct += loc_pt_inds(i, 1); },
      npts_check);
  assert(npts_check == npts);
  std::cout << "NodeArrayD::init: npoints check = "
            << (npts_check == npts ? " pass\n" : "FAIL\n");
#endif
};
...
std::string NodeArrayD::infoString(const bool& verbose) const {
  ...
  if (verbose) {
    ...
    for (Index i = 0; i < node_keys.extent(0); ++i) {
      ss << "\t\t" << i << ": key = " << std::bitset<32>(keys(i))
         << " pt_start = " << pt_inds(i, 0) << " pt_ct = " << pt_inds(i, 1)
         << " parent key = " << parent_key(keys(i), depth, depth)
         << " parent index = " << parents(i) << "\n";
    }
    //         ss << "\tpoints:\n";
    //         for (Index i=0; i<pt_in_node.extent(0); ++i) {
    //             ss << "\t\t" << i << ": in node " << pt_node(i) << " old_id =
    //             " << old_ids(i) << "\n";
    //         }
  }
  return ss.str();
