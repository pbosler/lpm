...
struct EncodeFunctor {
  ...
  EncodeFunctor(ko::View<code_type*>& co, const ko::View<Real* [3]>& p,
                const ko::View<BBox>& b, const Int& md)
...
struct PermuteFunctor {
  ...
  PermuteFunctor(ko::View<Real* [3]>& op, ko::View<Index*>& oi,
                 const ko::View<Real* [3]>& ip, const ko::View<code_type*>& c)
      : outpts(op), orig_inds(oi), inpts(ip), codes(c) {}
...
struct UnpermuteFunctor {
  ...
  UnpermuteFunctor(ko::View<Real* [3]>& op, const ko::View<Real* [3]>& ip,
                   const ko::View<Index*>& oi)
...
/**
    ...
    Step 1: Flag
        Loop over sorted codes.  Set flag = 1 if new node key is found, 0
   otherwise. Step 2: Scan Scan flags (inclusive) After scan, flag(npts-1)+1 =
...
struct MarkDuplicates {
  ...
  MarkDuplicates(ko::View<Index*> f, const ko::View<code_type*>& c)
      : flags(f), codes(c) {}
  ...
  KOKKOS_INLINE_FUNCTION
  void operator()(const ScanTag&, const Index& i, Index& ct,
...
/**
    Collect data about each unique node, to be used later to construct the nodes
   in NodeArray.

    input = output of MarkDuplicates kernel's 2 steps.

    output = array containing unique node keys
        node_ind = flag(i) if flag(i) is a new node.  Otherwise, the thread is
   idle. for each node key, 2 indices: inds_out(node_ind, 0) = index of first
   point (in sorted_pts) contained by node inds_out(node_ind, 1) = count of
   points contained by node
*/
...
/**
    For later nearest-neighbor searches, we want to add the siblings of every
   unique node, even if they're empty, so that each parent will have a full set
   of 8 child nodes.
...
struct NodeSiblingCounter {
  ...
  NodeSiblingCounter(ko::View<Index*> na, const ko::View<key_type*>& kk,
                     const Int& ll, const Int& md)
...
/**
    ...
    For each unique parent, construct full set of 8 children; some of them may
   be empty (contain no points)
*/
struct NodeArrayDFunctor {
  ...
  NodeArrayDFunctor(ko::View<key_type*>& nk, ko::View<Index* [2]>& np,
                    ko::View<Index*>& nprts, ko::View<Index*>& pinn,
                    const ko::View<Index*>& ns, const ko::View<key_type*>& uk,
                    const ko::View<Index* [2]>& ui, const Int& d)
      : node_keys(nk),
        node_pt_inds(np),
        node_parents(nprts),
        pt_in_node(pinn),
    ...
    if (new_parent) {
      ...
      for (int j = 0; j < 8; ++j) {
        ...
        if (found_key != NULL_IND) {
          ...
          for (Index k = points_start_ind; k < points_start_ind + points_count;
               ++k) {
...
struct ParentNodeFunctor {
  ...
  ParentNodeFunctor(ko::View<key_type*>& ko, ko::View<Index* [2]>& io,
                    const ko::View<key_type*>& kl,
                    const ko::View<Index* [2]>& il, const Int& lev,
                    const Int& md)
      : keys_out(ko),
        inds_out(io),
        keys_from_lower(kl),
  ...
  void operator()(const Index& i) const {
    // i in [0, nparents-1]
    const Index my_first_kid =
        8 * i;  // address of first kid in lower level arrays
    const key_type my_key =
        parent_key(keys_from_lower(my_first_kid), lower_level, max_depth);
    keys_out(i) = my_key;
    inds_out(i, 1) = 0;
    for (int j = 0; j < 8; ++j) {
...
struct NodeArrayInternalFunctor {
  ...
  NodeArrayInternalFunctor(ko::View<key_type*>& nkeys,
                           ko::View<Index* [2]>& npi, ko::View<Index*>& npts,
                           ko::View<Index* [8]>& nkids, ko::View<Index*>& plow,
                           const ko::View<Index*>& nsibs,
                           const ko::View<key_type*>& uk,
                           const ko::View<Index* [2]>& ui,
                           const ko::View<key_type*>& klow, const Int& lev,
                           const Int& max)
      : node_keys(nkeys),
        node_pt_inds(npi),
        node_parents(npts),
        node_kids(nkids),
        parents_from_lower(plow),
        nsiblings(nsibs),
        ukeys(uk),
        uinds(ui),
        keys_from_lower(klow),
        level(lev),
        max_depth(max) {}
  ...
  void operator()(const Index& i) const {
    bool new_parent =
        true;  // true if nodes at this level have different parents
    if (i > 0) new_parent = (nsiblings(i) > nsiblings(i - 1));
    if (new_parent) {
      const key_type pkey = parent_key(
          ukeys(i), level, max_depth);  // key of common parent at next level up
      const Index kid0_address =
          nsiblings(i) - 8;  // index of new parent's first child at this level
      for (int j = 0; j < 8; ++j) {
        const Index node_ind =
            kid0_address + j;  // index of new node at this level
        const key_type new_key = node_key(
            pkey, j, level, max_depth);  // key of new node at this level
        node_keys(node_ind) = new_key;
        node_parents(node_ind) = NULL_IND;
        const Index found_key = binarySearchKeys(new_key, ukeys, true);
        if (found_key != NULL_IND) {  // this sibling is nonempty
          ...
          const Index kid0_lower =
              binarySearchKeys(new_key, keys_from_lower,
                               true);  // index of first child at lower level
          for (int k = 0; k < 8; ++k) {
...
struct NeighborhoodFunctor {
  ...
  NeighborhoodFunctor(ko::View<Index* [27]>& n, const ko::View<key_type*>& k,
                      const ko::View<Index* [8]>& c, const ko::View<Index*>& p,
  ...
  void operator()(const Index& t) const {
    ...
    for (int j = 0; j < 27; ++j) {
      ...
      neighbors(t, j) =
          (h != NULL_IND ? kids(h, table_val(i, j, ctable)) : NULL_IND);
    }
  }
};
...
struct VertexSetupFunctor {
  ...
  VertexSetupFunctor(ko::View<Index* [8]>& o, ko::View<Int* [8]>& f,
                     ko::View<Int*>& nvan, ko::View<Index*>& a,
                     const ko::View<key_type*>& k,
                     const ko::View<Index* [27]>& nn, const Index& lo)
      : owner(o),
        flags(f),
        nverts_at_node(nvan),
        address(a),
  ...
  void operator()(const OwnerTag&, const Index& t) const {
    Int nv = 0;
    for (int i = 0; i < 8; ++i) {  // loop over node t's vertices (future: this
                                   // loop can be flattened)
  ...
  KOKKOS_INLINE_FUNCTION
  void operator()(const ScanTag&, const Index& t, Int& ct,
                  const bool& final_pass) const {
...
struct VertexFunctor {
  ...
  VertexFunctor(ko::View<Index* [8]>& o, ko::View<Int* [8]>& f,
                ko::View<Int*>& nnv, ko::View<Index*>& va,
                ko::View<Index* [8]>& vn, ko::View<Index* [8]>& nv,
                const ko::View<Index* [27]>& nn)
      : owner(o),
        flags(f),
        nverts_at_node(nnv),
        vertex_address(va),
  ...
  void operator()(const BuildTag&, const Index& t) const {
    ...
    for (int i = 0; i < 8; ++i) {
      if (flags(t, i)) {  // node t owns its ith vertex
        ...
        // build the vertex
        for (int j = 0; j < 8; ++j) {
          vertex_nodes(start_ind + loc_ind, j) =
              neighbors(t, table_val(i, j, nvtable));
        }
        loc_ind++;
      }
    }
