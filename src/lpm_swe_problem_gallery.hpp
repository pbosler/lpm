...
struct PlanarGravityWaveFreeBoundaries {
  ...
  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real sfc0(const PtType& xy) const {
    return sfc_height +
           sfc_ptb_max *
  ...
  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real zeta0(const PtType& xy) const {
    return 0;
  }

  template <typename PtType, typename VecType>
  KOKKOS_INLINE_FUNCTION void u0(VecType& uv, const PtType& xy) const {
    uv[0] = 0;
    uv[1] = 0;
  }

  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real sigma0(const PtType& xy) const {
    return 0;
  }
};
...
struct NitscheStricklandVortex {
  ...
  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real h0(const PtType& xy) const {
    return sfc0(xy);
  }
...
struct SWETestCase2 {
  ...
  KOKKOS_INLINE_FUNCTION
  SWETestCase2(const Real hh = 1, const Real Omg = 2 * constants::PI,
               const Real uu = constants::PI / 6)
      : h0max(hh), Omega(Omg), u0max(uu) {}

  template <typename VecType, typename PtType>
  KOKKOS_INLINE_FUNCTION void u0(VecType& uvw, const PtType& xyz) const {
  ...
  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real zeta0(const PtType& xyz) const {
    return 2 * u0max * xyz[2];
  }

  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real sfc0(const PtType& xyz) const {
    return h0max - (Omega * u0max + 0.5 * square(u0max)) * square(xyz[2]) /
                       constants::GRAVITY;
  }

  template <typename PtType>
  KOKKOS_INLINE_FUNCTION Real h0(const PtType& xyz) const {
    return sfc0(xyz);
  }
