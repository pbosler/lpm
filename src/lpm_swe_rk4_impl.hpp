...
struct SWERK4Update {
  ...
  SWERK4Update(crd_view& x_inout, const crd_view x1, const crd_view x2,
               const crd_view x3, const crd_view x4,
               scalar_view_type& zeta_inout, const scalar_view_type zeta1,
               const scalar_view_type zeta2, const scalar_view_type zeta3,
               const scalar_view_type zeta4, scalar_view_type& sigma_inout,
               const scalar_view_type sigma1, const scalar_view_type sigma2,
               const scalar_view_type sigma3, const scalar_view_type sigma4,
               scalar_view_type& h_or_a_inout, const scalar_view_type h_or_a1,
               const scalar_view_type h_or_a2, const scalar_view_type h_or_a3,
               const scalar_view_type h_or_a4)
      : x_inout(x_inout),
        x1(x1),
        x2(x2),
        x3(x3),
  ...
  void operator()(const Index i) const {
    for (int j = 0; j < Geo::ndim; ++j) {
      x_inout(i, j) +=
          sixth * (x1(i, j) + x4(i, j)) + third * (x2(i, j) + x3(i, j));
    }
    zeta_inout(i) +=
        sixth * (zeta1(i) + zeta4(i)) + third * (zeta2(i) + zeta3(i));
    sigma_inout(i) +=
        sixth * (sigma1(i) + sigma4(i)) + third * (sigma2(i) + sigma3(i));
    h_or_a_inout(i) +=
        sixth * (h_or_a1(i) + h_or_a4(i)) + third * (h_or_a2(i) + h_or_a3(i));
  }
};

// constructor
template <typename SeedType, typename TopoType>
SWERK4<SeedType, TopoType>::SWERK4(const Real timestep, SWE<SeedType>& swe,
                                   TopoType& topo)
    : dt(timestep),
      t_idx(0),
      swe(swe),
  ...
  passive_policy = std::make_unique<Kokkos::TeamPolicy<>>(
      swe.mesh.n_vertices_host(), Kokkos::AUTO());
  active_policy = std::make_unique<Kokkos::TeamPolicy<>>(
      swe.mesh.n_faces_host(), Kokkos::AUTO());
  set_fixed_views();
}
...
void SWERK4<SeedType, TopoType>::set_fixed_views() {
  ...
  passive_du1dx1     = swe.du1dx1_passive.view;
  passive_du1dx2     = swe.du1dx2_passive.view;
  passive_du2dx1     = swe.du2dx1_passive.view;
  passive_du2dx2     = swe.du2dx2_passive.view;
  ...
  active_du1dx1     = swe.du1dx1_active.view;
  active_du1dx2     = swe.du1dx2_active.view;
  active_du2dx1     = swe.du2dx1_active.view;
  active_du2dx2     = swe.du2dx2_active.view;
...
template <typename SeedType, typename TopoType>
void SWERK4<SeedType, TopoType>::advance_timestep_impl() {
  return advance_timestep(
      swe.mesh.vertices.phys_crds.view, swe.rel_vort_passive.view,
      swe.div_passive.view, swe.depth_passive.view, swe.velocity_passive.view,
      swe.mesh.faces.phys_crds.view, swe.rel_vort_active.view,
      swe.div_active.view, swe.depth_active.view, swe.velocity_active.view,
      swe.mesh.faces.area, swe.mass_active.view, swe.mesh.faces.mask);
}

template <typename SeedType, typename TopoType>
void SWERK4<SeedType, TopoType>::advance_timestep(
    crd_view& vx, scalar_view_type& vzeta, scalar_view_type& vsigma,
    scalar_view_type& vh, vec_view& vvel, crd_view& fx, scalar_view_type& fzeta,
    scalar_view_type& fsigma, scalar_view_type& fh, vec_view& fvel,
    scalar_view_type& farea, const scalar_view_type& fmass,
  ...
  Kokkos::parallel_for(
      "RK4-1 vertex tendencies", nverts,
      SWEVorticityDivergenceHeightTendencies<geo>(
          passive_rel_vort1, passive_div1, passive_depth1, passive_x,
          passive_vel, passive_rel_vort, passive_divergence, passive_depth,
          passive_ddot, passive_laps, swe.coriolis, swe.g, dt));
  /// compute tendencies for zeta, sigma, area (active particles)
  Kokkos::parallel_for(
      "RK4-1 face tendencies", nfaces,
      SWEVorticityDivergenceAreaTendencies<geo>(
          active_rel_vort1, active_div1, active_area1, active_x, active_vel,
          active_rel_vort, active_divergence, active_area, active_ddot,
          active_laps, swe.coriolis, swe.g, dt));
  ...
  KokkosBlas::update(1.0, passive_rel_vort, 0.5, passive_rel_vort1, 0,
                     passive_rel_vortwork);
  KokkosBlas::update(1.0, active_rel_vort, 0.5, active_rel_vort1, 0,
                     active_rel_vortwork);
  /// update input divergence to time t + 0.5*dt
  KokkosBlas::update(1.0, passive_divergence, 0.5, passive_div1, 0,
                     passive_divwork);
  KokkosBlas::update(1.0, active_divergence, 0.5, active_div1, 0,
                     active_divwork);
  /// update input depth to time t + 0.5*dt
  KokkosBlas::update(1.0, passive_depth, 0.5, passive_depth1, 0,
                     passive_depthwork);
  ...
  Kokkos::parallel_for("RK4-2 surface input, passive", nverts,
                       SetSurfaceFromDepth<typename SeedType::geo, TopoType>(
                           passive_surface, passive_bottom, passive_xwork,
                           passive_depthwork, topo));
  Kokkos::parallel_for(
      "RK4-2, surface input, active", nfaces,
      SetDepthAndSurfaceFromMassAndArea<typename SeedType::geo, TopoType>(
          active_depth, active_surface, active_bottom, active_xwork,
          active_mass, active_areawork, active_mask, topo));
  /// compute velocity, double dot, and surface laplacian
  Kokkos::parallel_for(
      "RK4-2 direct sum, passive", *passive_policy,
      PlanarSWEVertexSums(passive_vel, passive_ddot, passive_du1dx1,
                          passive_du1dx2, passive_du2dx1, passive_du2dx2,
                          passive_laps, passive_psi, passive_phi, passive_xwork,
                          passive_surface, active_xwork, active_rel_vortwork,
                          active_divwork, active_areawork, active_mask,
                          active_surface, eps, pse_eps, nfaces, do_velocity));
  Kokkos::parallel_for(
      "RK4-2 direct sum, active", *active_policy,
      PlanarSWEFaceSums(active_vel, active_ddot, active_du1dx1, active_du1dx2,
                        active_du2dx1, active_du2dx2, active_laps, active_psi,
                        active_phi, active_xwork, active_rel_vortwork,
                        active_divwork, active_areawork, active_mask,
                        active_surface, eps, pse_eps, nfaces, do_velocity));
  /// compute vorticity, divergence, and height tendencies
  Kokkos::parallel_for(
      "RK4-2 passive tendencies", nverts,
      SWEVorticityDivergenceHeightTendencies<geo>(
          passive_rel_vort2, passive_div2, passive_depth2, passive_xwork,
          passive_vel, passive_rel_vortwork, passive_divwork, passive_depthwork,
          passive_ddot, passive_laps, swe.coriolis, swe.g, dt));
  Kokkos::parallel_for(
      "RK4-2 active tendencies", nfaces,
      SWEVorticityDivergenceAreaTendencies<geo>(
          active_rel_vort2, active_div2, active_area2, active_xwork, active_vel,
          active_rel_vortwork, active_divwork, active_areawork, active_ddot,
          active_laps, swe.coriolis, swe.g, dt));
  ...
  KokkosBlas::update(1.0, passive_rel_vort, 0.5, passive_rel_vort2, 0,
                     passive_rel_vortwork);
  KokkosBlas::update(1.0, active_rel_vort, 0.5, active_rel_vort2, 0,
                     active_rel_vortwork);
  KokkosBlas::update(1.0, passive_divergence, 0.5, passive_div2, 0,
                     passive_divwork);
  KokkosBlas::update(1.0, active_divergence, 0.5, active_div2, 0,
                     active_divwork);
  KokkosBlas::update(1.0, passive_depth, 0.5, passive_depth2, 0,
                     passive_depthwork);
  KokkosBlas::update(1.0, active_area, 0.5, active_area2, 0, active_areawork);
  Kokkos::parallel_for("RK4-3 surface input, passive", nverts,
                       SetSurfaceFromDepth<typename SeedType::geo, TopoType>(
                           passive_surface, passive_bottom, passive_xwork,
                           passive_depthwork, topo));
  Kokkos::parallel_for(
      "RK4-3, surface input, active", nfaces,
      SetDepthAndSurfaceFromMassAndArea<typename SeedType::geo, TopoType>(
          active_depth, active_surface, active_bottom, active_xwork,
          active_mass, active_areawork, active_mask, topo));
  Kokkos::parallel_for(
      "RK4-3 direct sum, passive", *passive_policy,
      PlanarSWEVertexSums(passive_vel, passive_ddot, passive_du1dx1,
                          passive_du1dx2, passive_du2dx1, passive_du2dx2,
                          passive_laps, passive_psi, passive_phi, passive_xwork,
                          passive_surface, active_xwork, active_rel_vortwork,
                          ...
                          active_surface, eps, pse_eps, nfaces, do_velocity));
  Kokkos::parallel_for(
      "RK4-3 direct sum, active", *active_policy,
      PlanarSWEFaceSums(active_vel, active_ddot, active_du1dx1, active_du1dx2,
                        active_du2dx1, active_du2dx2, active_laps, active_psi,
                        active_phi, active_xwork, active_rel_vortwork,
                        active_divwork, active_areawork, active_mask,
                        active_surface, eps, pse_eps, nfaces, do_velocity));
  Kokkos::parallel_for(
      "RK4-3 passive tendencies", nverts,
      SWEVorticityDivergenceHeightTendencies<geo>(
          passive_rel_vort3, passive_div3, passive_depth3, passive_xwork,
          passive_vel, passive_rel_vortwork, passive_divwork, passive_depthwork,
          passive_ddot, passive_laps, swe.coriolis, swe.g, dt));
  Kokkos::parallel_for(
      "RK4-3 active tendencies", nfaces,
      SWEVorticityDivergenceAreaTendencies<geo>(
          active_rel_vort3, active_div3, active_area3, active_xwork, active_vel,
          active_rel_vortwork, active_divwork, active_areawork, active_ddot,
          active_laps, swe.coriolis, swe.g, dt));
  ...
  KokkosBlas::update(1.0, passive_rel_vort, 1.0, passive_rel_vort3, 0,
                     passive_rel_vortwork);
  KokkosBlas::update(1.0, active_rel_vort, 1.0, active_rel_vort3, 0,
                     active_rel_vortwork);
  KokkosBlas::update(1.0, passive_divergence, 1.0, passive_div3, 0,
                     passive_divwork);
  KokkosBlas::update(1.0, active_divergence, 1.0, active_div3, 0,
                     active_divwork);
  KokkosBlas::update(1.0, passive_depth, 1.0, passive_depth3, 0,
                     passive_depthwork);
  KokkosBlas::update(1.0, active_area, 1.0, active_area3, 0, active_areawork);
  Kokkos::parallel_for("RK4-4 surface input, passive", nverts,
                       SetSurfaceFromDepth<typename SeedType::geo, TopoType>(
                           passive_surface, passive_bottom, passive_xwork,
                           passive_depthwork, topo));
  Kokkos::parallel_for(
      "RK4-4, surface input, active", nfaces,
      SetDepthAndSurfaceFromMassAndArea<typename SeedType::geo, TopoType>(
          active_depth, active_surface, active_bottom, active_xwork,
          active_mass, active_areawork, active_mask, topo));
  Kokkos::parallel_for(
      "RK4-4 direct sum, passive", *passive_policy,
      PlanarSWEVertexSums(passive_vel, passive_ddot, passive_du1dx1,
                          passive_du1dx2, passive_du2dx1, passive_du2dx2,
                          passive_laps, passive_psi, passive_phi, passive_xwork,
                          passive_surface, active_xwork, active_rel_vortwork,
                          ...
                          active_surface, eps, pse_eps, nfaces, do_velocity));
  Kokkos::parallel_for(
      "RK4-4 direct sum, active", *active_policy,
      PlanarSWEFaceSums(active_vel, active_ddot, active_du1dx1, active_du1dx2,
                        active_du2dx1, active_du2dx2, active_laps, active_psi,
                        active_phi, active_xwork, active_rel_vortwork,
                        active_divwork, active_areawork, active_mask,
                        active_surface, eps, pse_eps, nfaces, do_velocity));
  Kokkos::parallel_for(
      "RK4-4 passive tendencies", nverts,
      SWEVorticityDivergenceHeightTendencies<geo>(
          passive_rel_vort4, passive_div4, passive_depth4, passive_xwork,
          passive_vel, passive_rel_vortwork, passive_divwork, passive_depthwork,
          passive_ddot, passive_laps, swe.coriolis, swe.g, dt));
  Kokkos::parallel_for(
      "RK4-4 active tendencies", nfaces,
      SWEVorticityDivergenceAreaTendencies<geo>(
  ...
  Kokkos::parallel_for(
      "RK4 output, passive", nverts,
      SWERK4Update<PlaneGeometry>(
          passive_x, passive_x1, passive_x2, passive_x3, passive_x4,
          passive_rel_vort, passive_rel_vort1, passive_rel_vort2,
          passive_rel_vort3, passive_rel_vort4, passive_divergence,
          passive_div1, passive_div2, passive_div3, passive_div4, passive_depth,
          passive_depth1, passive_depth2, passive_depth3, passive_depth4));
  Kokkos::parallel_for(
      "RK4 output, active", nfaces,
      SWERK4Update<PlaneGeometry>(
          active_x, active_x1, active_x2, active_x3, active_x4, active_rel_vort,
          active_rel_vort1, active_rel_vort2, active_rel_vort3,
          active_rel_vort4, active_divergence, active_div1, active_div2,
          active_div3, active_div4, active_area, active_area1, active_area2,
          active_area3, active_area4));
  Kokkos::parallel_for(
      "RK4-final surface, passive", nverts,
      SetSurfaceFromDepth<typename SeedType::geo, TopoType>(
          passive_surface, passive_bottom, passive_x, passive_depth, topo));
  Kokkos::parallel_for(
      "RK4-4-final, surface, active", nfaces,
      SetDepthAndSurfaceFromMassAndArea<typename SeedType::geo, TopoType>(
          active_depth, active_surface, active_bottom, active_x, active_mass,
          active_area, active_mask, topo));
  Kokkos::parallel_for(
      "RK4-final direct sum, passive", *passive_policy,
      PlanarSWEVertexSums(passive_vel, passive_ddot, passive_du1dx1,
                          passive_du1dx2, passive_du2dx1, passive_du2dx2,
                          passive_laps, passive_psi, passive_phi, passive_x,
                          passive_surface, active_x, active_rel_vort,
                          active_divergence, active_area, active_mask,
                          active_surface, eps, pse_eps, nfaces, do_velocity));
  Kokkos::parallel_for(
      "RK4-final direct sum, active", *active_policy,
      PlanarSWEFaceSums(active_vel, active_ddot, active_du1dx1, active_du1dx2,
                        active_du2dx1, active_du2dx2, active_laps, active_psi,
                        active_phi, active_x, active_rel_vort,
                        active_divergence, active_area, active_mask,
                        active_surface, eps, pse_eps, nfaces, do_velocity));
