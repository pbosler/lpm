...
void biotSavartSphere(TgtVec& u, const SrcVec& x, const SrcVec& xx, const Real& zeta, const Real& area, const Real& smoother = 0) {
    const Real strength = -4*PI / (1 - dot(x,xx) + square(smoother)) * zeta * area;
    SphereGeometry::cross(u, x, xx);
    SphereGeometry::scale(u, strength);
}

struct BVEDirectSumCollocated {
    typedef Real value_type[3];
...
struct BVEVertexVelocity {
    ...
    void operator() (const Index& i) const {
        Real vel[3] = {0,0,0};
        ko::parallel_reduce(n, BVEDirectSumDistinct(i, physFaceCrds, faceRelVort, faceArea,
...
struct BVEFaceVelocity {
    ...
    void operator() const (Index& i) const {
        Real vel[3] = {0,0,0};
        ko::parallel_reduce(n, BVEDirectSumCollocated(i, physFaceCrds, faceRelVort, faceArea), vel);
        for (int j=0; j<3; ++j) {
...
struct BVERK4 : public RK4<3,BVEVertexVelocity, BVEFaceVelocity> {
    ...
    BVERK4(vec_view_type vs, scalar_view_type vz, vec_view_type fs, vec_view_type vv, vec_view_type fv, scalar_view_type fz, 
        scalar_view_type fa, const Real omega) : RK4<3,BVEVertexVelocity,BVEFaceVelocity>(ns, fs, vv, fv, fz, fa), vertVelocity(vz),
    ...
    void timestep(const Real dt) const override {
        ...
        ko::parallel_for(nv, BVEVertexVelocity(nf, vertcrds, facecrds, fconvolution, facearea, vertstage1));
        ko::parallel_for(nf, BVEFaceVelocity(nf, facecrds, fconvolution, facearea, facestage1));
        /// stage 1 vorticity
        ...
        ko::parallel_for(nf, StageSetup(facecrds, faceinput, fconvolution, facevortin, facestage1, facevortstage1, 2, dt));
        /// stage 2 velocity
        ko::parallel_for(nv, BVEVertexVelocity(nf, vertinput, faceinput, facevortin, facearea, vertstage2));
        ko::parallel_for(nf, BVEFaceVelocity(nf, faceinput, facevortin, facearea, facestage2));
        /// stage 2 vorticity
        ...
        ko::parallel_for(nf, StageSetup(facecrds, faceinput, fconvolution, facevortin, facestage2, facevortstage2, 3, dt));
        /// stage 3 velocity
        ko::parallel_for(nv, BVEVertexVelocity(nf, vertinput, faceinput, facevortin, facearea, vertstage3));
        ko::parallel_for(nf, BVEFaceVelocity(nf, faceinput, facevortin, facearea, facestage3));
        /// stage 3 vorticity
        ...
        ko::parallel_for(nf, StageSetup(facecrds, faceinput, fconvolution, facevortin, facestage3, facevortstage3, 4, dt));
        /// stage 4 velocity
        ko::parallel_for(nv, BVEVertexVelocity(nf, vertinput, faceinput, facevortin, facearea, vertstage4));
        ko::parallel_for(nf, BVEFaceVelocity(nf, faceinput, facevortin, facearea, facestage4));
        /// stage 4 vorticity
        ...
        ko::parallel_for(nf, KOKKOS_LAMBDA (const Index i) {
            ...
            fconvolution(i) += dto6*facevortstage1(i) + dto3*facevortstage2(i) + dto3*facevortstage3(i) + dto6*facevortstage4(i);
        });
    } 
};
