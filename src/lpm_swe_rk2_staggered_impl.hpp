...
struct StaggeredSWETendencies {
  ...
  StaggeredSWETendencies(
      scalar_view_type& dz, scalar_view_type& ds, scalar_view_type& da,
      const crd_view x, const vec_view u, const scalar_view_type zeta,
      const scalar_view_type sigma, const scalar_view_type area,
      const scalar_view_type gfcu, const scalar_view_type ddot,
      const scalar_view_type laps, const mask_view_type mask,
      const coriolis_type& coriolis, const Real g, const Real dt)
      : dzeta(dz),
        dsigma(ds),
        darea(da),
        x(x),
  ...
  void operator()(const Index i) const {
    if (!mask(i)) {
      ...
      dsigma(i) =
          (f * zeta(i) + grad_f_cross_u(i) - ddot(i) - g * laps(i)) * dt;
...
SWERK2Staggered<SeedType, TopoType>::SWERK2Staggered(
    const Real timestep, StaggeredSWE<SeedType, TopoType>& swe,
  ...
  vertex_policy = std::make_unique<Kokkos::TeamPolicy<>>(
...
void SWERK2Staggered<SeedType, TopoType>::advance_timestep_impl() {
  ...
  auto velocity_view =
      Kokkos::subview(swe.velocity.view, vert_range, Kokkos::ALL);
  auto velocity_avg_view =
      Kokkos::subview(swe.velocity_avg.view, face_range, Kokkos::ALL);
  auto x_view = Kokkos::subview(swe.mesh.vertices.phys_crds.view, vert_range,
                                Kokkos::ALL);
  auto x_avg_view =
      Kokkos::subview(swe.mesh.faces.phys_crds.view, face_range, Kokkos::ALL);
  ...
  auto vert_view =
      Kokkos::subview(swe.mesh.faces.verts, face_range, Kokkos::ALL);
  ...
  Kokkos::parallel_for(
      "RK2-1 tendencies", nfaces,
      StaggeredSWETendencies<typename SeedType::geo>(
          rel_vort1, divergence1, area1, x_avg_view, velocity_avg_view,
          zeta_view, sigma_view, area_view, gfcu_avg_view, ddot_avg_view,
          slap_view, mask_view, swe.coriolis, swe.g, dt));
  ...
  Kokkos::parallel_for(
      nfaces, FacePositionAverages<SeedType>(x_avg_view, xwork, vert_view));

  Kokkos::parallel_for(
      "RK2-2 surface input", nfaces,
      SetDepthAndSurfaceFromMassAndArea<typename SeedType::geo, TopoType>(
          depth_view, s_view, b_view, x_avg_view, mass_view, areawork,
          mask_view, swe.topography));
  swe.gmls_surface_laplacian(x_avg_view, gmls_params);

  Kokkos::parallel_for(
      *vertex_policy,
      SphereVertexSums(velocity_view, ddot_view, xwork, x_avg_view,
                       rel_vortwork, divergencework, areawork, mask_view, eps,
                       nfaces, do_velocity));
  KokkosBlas::scal(x2, dt, velocity_view);

  Kokkos::parallel_for(
      nverts, GradFCrossU<typename SeedType::geo>(gfcu_view, xwork,
                                                  velocity_view, swe.coriolis));
  Kokkos::parallel_for(
      nfaces, VertexToFaceAverages<SeedType>(
                  x_avg_view, ddot_avg_view, gfcu_avg_view, velocity_avg_view,
                  xwork, ddot_view, gfcu_view, velocity_view, vert_view));

  Kokkos::parallel_for(
      "RK2-2 tendencies", nfaces,
      StaggeredSWETendencies<typename SeedType::geo>(
          rel_vort2, divergence2, area2, x_avg_view, velocity_avg_view,
          zeta_view, sigma_view, area_view, gfcu_avg_view, ddot_avg_view,
          slap_view, mask_view, swe.coriolis, swe.g, dt));
  ...
  Kokkos::parallel_for(
      nfaces, FacePositionAverages<SeedType>(x_avg_view, x_view, vert_view));
  Kokkos::parallel_for(
      nfaces,
      SetDepthAndSurfaceFromMassAndArea<typename SeedType::geo, TopoType>(
          depth_view, s_view, b_view, x_avg_view, mass_view, area_view,
          mask_view, swe.topography));
  swe.gmls_surface_laplacian(x_avg_view, gmls_params);

  Kokkos::parallel_for(
      *vertex_policy,
      SphereVertexSums(velocity_view, ddot_view, x_view, x_avg_view, zeta_view,
                       sigma_view, area_view, mask_view, eps, nfaces,
                       do_velocity));
  Kokkos::parallel_for(
      nverts, GradFCrossU<typename SeedType::geo>(gfcu_view, x_view,
                                                  velocity_view, swe.coriolis));
  Kokkos::parallel_for(
      nfaces, VertexToFaceAverages<SeedType>(
                  x_avg_view, ddot_avg_view, gfcu_avg_view, velocity_avg_view,
                  x_view, ddot_view, gfcu_view, velocity_view, vert_view));
