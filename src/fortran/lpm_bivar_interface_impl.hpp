...
void c_idbvip(int md, int nsrc, double* xin, double* yin, double* zin,
  int ndst, double* x_out, double* y_out, double* zout, int* iwk, double* rwk) {
...
{
  LPM_REQUIRE_MSG(in.unpacked, "BivarInterface error: GatheredData must have unpacked coordinates.");
...
template <typename SeedType>
void BivarInterface<SeedType>::interpolate(const sfield_map& output_scalars, const vfield_map& output_vectors) {
  ...
  for (const auto& io : scalar_in_out_map) {
    ...
    LPM_REQUIRE_MSG( output_scalars.count(io.second) == 1, "BivarInterface::interpolate error: output scalar field " + io.second + " not found." );
    const auto zout = output_scalars.at(io.second);

    c_idbvip(md, nsrc, xin.data(), yin.data(), zin.data(),
      ndst, x_out.data(), y_out.data(), zout.data(),
      integer_work.data(), real_work.data());
    md = 3; /// Tell bivar.f90 that the input data will not change
  }
  /// todo: testing required --- is it faster to interpolate one component at a time, or one pair of components individually, in a loop?
  //// to start, we do one component at a time, which requires an extra deep copy
  ...
  if ( !vector_in_out_map.empty() ) {
    ...
    for (const auto& io : vector_in_out_map) {
      ...
      LPM_REQUIRE_MSG( output_vectors.count(io.second) == 1,
        "BivarInterface::interpolate error: output vector field " + io.second + " not found.");

      for (int j=0; j<SeedType::geo::ndim; ++j) {
        /// get the input component subview (generally, it will have strided access)
        ...
        c_idbvip(md, nsrc, xin.data(), yin.data(), vec_comp_in.data(),
          ndst, x_out.data(), y_out.data(), vec_comp_out.data(),
          integer_work.data(), real_work.data());
        /// Tell bivar.f90 that the input data will not change
        md = 3;
        /// get the output component subview (generally, it will have strided access)
        const auto vco = Kokkos::subview(output_vectors.at(io.second),
          Kokkos::ALL, j);
