...
std::string convergence_table(const std::string dxlabel,
  ...
  ss << "\n";
  ss << std::setw(col_width) << dxlabel << std::setw(col_width) << exlabel
     << std::setw(col_width) << "rate\n";
  ss << std::setw(col_width) << dx[0] << std::setw(col_width) << ex[0]
     << std::setw(col_width) << "---\n";
  for (int i = 1; i < dx.size(); ++i) {
    ss << std::setw(col_width) << dx[i] << std::setw(col_width) << ex[i]
       << std::setw(col_width) << rate[i] << "\n";
  }
  return ss.str();
}

std::string ErrNorms::info_string(const std::string& label,
                                  const int tab_level) const {
  std::ostringstream ss;
  std::string tabstr;
  for (int i = 0; i < tab_level; ++i) tabstr += "\t";
  ss << tabstr << label << " ErrNorms: l1 = " << l1 << " l2 = " << l2
     << " linf = " << linf;
  return ss.str();
}
