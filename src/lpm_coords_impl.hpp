...
std::string Coords<Geo>::info_string(const std::string& label, const short& tab_level, const bool& dump_all) const {
...
template <typename Geo>
void Coords<Geo>::write_matlab(std::ostream& os, const std::string& name) const {
  os << name << " = [";
...
template <typename Geo> void Coords<Geo>::init_random(const Real max_range, const Int ss) {
  unsigned seed = 0 + ss;
...
void Coords<Geo>::init_interior_crds_from_seed(const MeshSeed<SeedType>& seed) {
  LPM_REQUIRE_MSG(_nmax >= SeedType::nfaces, "Coords::init_interior_crds_from_seed memory limit error.");
...
template <typename Geo>
typename Kokkos::MinMax<Real>::value_type Coords<Geo>::min_max_extent(const int dim) const {
  Kokkos::MinMax<Real>::value_type minmax;
  const auto local_crds = this->crds;
  Kokkos::parallel_reduce(_nh(), KOKKOS_LAMBDA (const Index i,
    Kokkos::MinMax<Real>::value_type& mm) {
