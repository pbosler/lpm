...
SWERK2<SeedType, TopoType>::SWERK2(const Real timestep, SWE<SeedType>& swe,
                                   TopoType& topo,
  ...
  gather->gather_scalar_fields(passive_field_map_gather,
                               active_field_map_gather);
  ...
  auto surf_gmls = gmls::sphere_scalar_gmls(
      gather->phys_crds, gather->phys_crds, neighbors, gmls_params, gmls_ops);
...
void SWERK2<SeedType, TopoType>::advance_timestep_impl() {
  ...
  Kokkos::parallel_for(
      "RK2-1 vertex tendencies", nverts,
      SWEVorticityDivergenceHeightTendencies<geo>(
          ...
          swe.rel_vort_passive.view, swe.div_passive.view,
          swe.depth_passive.view, swe.double_dot_passive.view,
          swe.surf_lap_passive.view, swe.coriolis, swe.g, dt));

  Kokkos::parallel_for(
      "RK2-1 face tendencies", nfaces,
      SWEVorticityDivergenceAreaTendencies<geo>(
          active_rel_vort1, active_div1, active_area1,
          swe.mesh.faces.phys_crds.view, swe.velocity_active.view,
          swe.rel_vort_active.view, swe.div_active.view, swe.mesh.faces.area,
          swe.double_dot_active.view, swe.surf_lap_active.view, swe.coriolis,
          swe.g, dt));

  /// RK Stage 2
  /// intermediate update to time t + dt
  KokkosBlas::update(1, swe.mesh.vertices.phys_crds.view, 1, passive_x1, 0,
                     passive_xwork);
  KokkosBlas::update(1, swe.rel_vort_passive.view, 1, passive_rel_vort1, 0,
                     passive_rel_vortwork);
  KokkosBlas::update(1, swe.div_passive.view, 1, passive_div1, 0,
                     passive_divwork);
  KokkosBlas::update(1, swe.mesh.faces.phys_crds.view, 1, active_x1, 0,
                     active_xwork);
  KokkosBlas::update(1, swe.rel_vort_active.view, 1, active_rel_vort1, 0,
                     active_rel_vortwork);
  KokkosBlas::update(1, swe.div_active.view, 1, active_div1, 0, active_divwork);
  KokkosBlas::update(1, swe.depth_passive.view, 1, passive_depth1, 0,
                     passive_depthwork);
  KokkosBlas::update(1, swe.mesh.faces.area, 1, active_area1, 0,
                     active_areawork);
  Kokkos::parallel_for("RK2-2 surface input, passive", nverts,
                       SetSurfaceFromDepth<geo, TopoType>(
                           swe.surf_passive.view, swe.bottom_passive.view,
                           passive_xwork, passive_depthwork, topo));
  Kokkos::parallel_for(
      "RK2-2 surface input, active", nfaces,
      SetDepthAndSurfaceFromMassAndArea<geo, TopoType>(
          swe.depth_active.view, swe.surf_active.view, swe.bottom_active.view,
          active_xwork, swe.mass_active.view, active_areawork,
          swe.mesh.faces.mask, topo));
  ...
  if constexpr (std::is_same<geo, SphereGeometry>::value) {
                                 ...
                                 active_field_map_gather);
    {
      const auto neighbors =
          gmls::Neighborhoods(gather->h_phys_crds, gmls_params);
      auto surf_gmls =
          gmls::sphere_scalar_gmls(gather->phys_crds, gather->phys_crds,
    ...
    scatter->scatter_fields(passive_field_map_scatter,
                            active_field_map_scatter);

    /// update velocity
    Kokkos::parallel_for(
        "RK2-2 direct sums, passive", *passive_policy,
        SphereVertexSums(swe.velocity_passive.view, swe.double_dot_passive.view,
                         passive_xwork, active_xwork, active_rel_vortwork,
                         active_divwork, active_areawork, swe.mesh.faces.mask,
                         eps, nfaces, do_velocity));
    Kokkos::parallel_for(
        "RK2-2 direct sums, active", *active_policy,
        SphereFaceSums(swe.velocity_active.view, swe.double_dot_active.view,
                       active_xwork, active_rel_vortwork, active_divwork,
                       active_areawork, swe.mesh.faces.mask, eps, nfaces,
                       do_velocity));

    Kokkos::parallel_for(
        "RK2-2 vertex tendencies", nverts,
        SWEVorticityDivergenceHeightTendencies<geo>(
            passive_rel_vort2, passive_div2, passive_depth2, passive_xwork,
            swe.velocity_passive.view, passive_rel_vortwork, passive_divwork,
            passive_depthwork, swe.double_dot_passive.view,
            swe.surf_lap_passive.view, swe.coriolis, swe.g, dt));

    Kokkos::parallel_for(
        "RK2-2 face tendencies", nfaces,
        SWEVorticityDivergenceAreaTendencies<geo>(
            active_rel_vort2, active_div2, active_area2, active_xwork,
            swe.velocity_active.view, active_rel_vortwork, active_divwork,
            active_areawork, swe.double_dot_active.view,
            swe.surf_lap_active.view, swe.coriolis, swe.g, dt));

    KokkosBlas::scal(passive_x2, dt, swe.velocity_passive.view);
    KokkosBlas::scal(active_x2, dt, swe.velocity_active.view);

    KokkosBlas::update(0.5, passive_x1, 0.5, passive_x2, 1,
                       swe.mesh.vertices.phys_crds.view);
    KokkosBlas::update(0.5, passive_rel_vort1, 0.5, passive_rel_vort2, 1,
                       swe.rel_vort_passive.view);
    KokkosBlas::update(0.5, passive_div1, 0.5, passive_div2, 1,
                       swe.div_passive.view);
    KokkosBlas::update(0.5, passive_depth1, 0.5, passive_depth2, 1,
                       swe.depth_passive.view);
    KokkosBlas::update(0.5, active_x1, 0.5, active_x2, 1,
                       swe.mesh.faces.phys_crds.view);
    KokkosBlas::update(0.5, active_rel_vort1, 0.5, active_rel_vort2, 1,
                       swe.rel_vort_active.view);
    KokkosBlas::update(0.5, active_div1, 0.5, active_div2, 1,
                       swe.div_active.view);
    KokkosBlas::update(0.5, active_area1, 0.5, active_area2, 1,
                       swe.mesh.faces.area);

    Kokkos::parallel_for(
        "RK2-2 surface update, passive", nverts,
        SetSurfaceFromDepth<geo, TopoType>(
            swe.surf_passive.view, swe.bottom_passive.view,
            swe.mesh.vertices.phys_crds.view, swe.depth_passive.view, topo));
    Kokkos::parallel_for(
        "RK2-2 surface update, active", nfaces,
        SetDepthAndSurfaceFromMassAndArea<geo, TopoType>(
            swe.depth_active.view, swe.surf_active.view, swe.bottom_active.view,
            swe.mesh.faces.phys_crds.view, swe.mass_active.view,
            swe.mesh.faces.area, swe.mesh.faces.mask, topo));

    Kokkos::parallel_for(
        "RK2-2 velocity update, passive", *passive_policy,
        SphereVertexSums(
            swe.velocity_passive.view, swe.double_dot_passive.view,
            swe.mesh.vertices.phys_crds.view, swe.mesh.faces.phys_crds.view,
            swe.rel_vort_active.view, swe.div_active.view, swe.mesh.faces.area,
            swe.mesh.faces.mask, eps, nfaces, do_velocity));

    Kokkos::parallel_for(
        "RK2-2 velocity update, active", *active_policy,
        SphereFaceSums(swe.velocity_active.view, swe.double_dot_active.view,
                       swe.mesh.faces.phys_crds.view, swe.rel_vort_active.view,
                       swe.div_active.view, swe.mesh.faces.area,
                       swe.mesh.faces.mask, eps, nfaces, do_velocity));

    gather->gather_coordinates(swe.mesh.vertices.phys_crds.view,
                               swe.mesh.faces.phys_crds.view);
    gather->update_host();
    gather->gather_scalar_fields(passive_field_map_gather,
                                 active_field_map_gather);
