...
template <typename VT = ko::View<Real*>>
KOKKOS_INLINE_FUNCTION void fourier_coeff_legendre_poly(const Int& n,
                                                        Real& constcoeff,
                                                        VT coeffs, VT dcoeffs);

template <typename CVT = ko::View<const Real*>>
void legendre_polynomial(const Int& n, const Real& theta,
                         const Real& constcoeff, const CVT coeffs,
                         const CVT dcoeffs, Real& polyval, Real& dpolyval);
...
struct GaussGrid {
  ...
  GaussGrid(const Int nl)
      : nlat(nl), colatitudes("colat", nl), weights("weights", nl) {
  ...
  switch (nlat) {
    ...
    case (2): {
      ...
      default: {
        ...
        fourier_coeff_legendre_poly<ko::View<Real*, MemSpace>>(
            nlat, const_coeff, coeffs, deriv_coeffs);
        ...
        while (root_ind >= 0) {
          ...
          while (keepGoing) {
            ...
            /// Newton iterations
            Real polyval, derivval;
            legendre_polynomial<ko::View<Real*, MemSpace>>(
                nlat, root_current, const_coeff, coeffs, deriv_coeffs, polyval,
                derivval);
            Real corr = polyval / derivval;
            Real sgn = 1.0;
            if (corr != 0.0) sgn = corr / std::abs(corr);
            corr = sgn * std::min(std::abs(corr), max_corr);
            std::cout << "iter " << iter << " p_n^0 = " << polyval
                      << " dp_n^0 = " << derivval << " corr = " << corr << "\n";
            root_current -= corr;
            std::cout << "rc = " << root_current << " rl = " << root_last
                      << " rp = " << root_prev << " ri = " << root_ind << "\n";
            if (std::abs(root_current - root_last) <=
                eps_roundoff * std::abs(root_current)) {
              std::cout << "converged!!!\n";
              keepGoing = false;
            }
          ...
          weights(root_ind) = (2 * nlat + 1) /
                              (square(derivval + polyval * std::cos(root_last) /
                                                     std::sin(root_last)));
          root_ind--;
        ...
        if (nlat % 2 != 0) {
          ...
          legendre_polynomial<ko::View<Real*, MemSpace>>(
              nlat, pio2, const_coeff, coeffs, deriv_coeffs, polyval, derivval);
...
/**
    Computes the Fourier coefficients a_{nk} of the Legendre polynomial P_n^0
   and its derivative, where
    ...
    a{nn} = \sqrt{\frac{2n+1}}{2} \cdot
   \frac{\Gamma(2n+1)}{2^{2n-1}\Gamma^2(n+1)}
...
/**
    Computes the value of the Legendre polynomial P_n(theta), where theta is
   colatitude, and its derivative
*/
