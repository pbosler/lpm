...
std::string CompadreParams::infoString(const int tab_level) const {
  ...
  ss << "\t" << tabstr
     << "gmls_manifold_weight_pow = " << gmls_manifold_weight_pow << '\n';
...
CompadreNeighborhoods::CompadreNeighborhoods(
    typename ko::View<Real* [3]>::HostMirror host_src_crds,
    typename ko::View<Real* [3]>::HostMirror host_tgt_crds,
    const CompadreParams& params) {
  Compadre::PointCloudSearch<typename ko::View<Real* [3]>::HostMirror>
      point_cloud_search(host_src_crds);
  const int est_upper_bound =
  ...
  point_cloud_search.generate2DNeighborListsFromKNNSearch(
      false, host_tgt_crds, host_neighbors, host_radii, params.min_neighbors,
...
std::string CompadreNeighborhoods::infoString(const int tab_level) const {
  ...
  ss << '\t' << tabstr
     << "neighbor_lists.extent(1) = " << neighbor_lists.extent(1) << "\n";
...
Compadre::GMLS scalarGMLS(
    const ko::View<Real* [3], DevMem> src_crds,
    const ko::View<Real* [3], DevMem> tgt_crds, const CompadreNeighborhoods& nn,
    const CompadreParams& params,
  ...
  Compadre::GMLS result(params.gmls_order, params.ambient_dim,
                        ...
                        constrain_name.c_str(), params.gmls_manifold_order);
...
Compadre::GMLS vectorGMLS(
  ...
  const std::string problem_name = "MANIFOLD";
  const std::string constrain_name = "NO_CONSTRAINT";
  Compadre::GMLS result(
      Compadre::ReconstructionSpace::VectorOfScalarClonesTaylorPolynomial,
      Compadre::ManifoldVectorPointSample, params.gmls_order,
      params.ambient_dim, solver_name.c_str(), problem_name.c_str(),
      constrain_name.c_str(), params.gmls_manifold_order);
  result.setProblemData(nn.neighbor_lists, src_crds, tgt_crds,
                        nn.neighborhood_radii);
  result.setReferenceOutwardNormalDirection(tgt_crds);
